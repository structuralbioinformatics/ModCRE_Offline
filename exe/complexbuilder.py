...
def parse_arguments():  # This function passes the arguments given by the user.
    ...
    aparser.add_argument('-r', action="store", default=5.0, dest="radius", help="Radius to calculate contacts. (Default is 4.0!)")
    aparser.add_argument('-dna', action="store_true", default=False,dest="dna",help="Use DNA to calculate contacts (default is False and DNA is not counted)")
...
def check_clash(structure_complex, moved_chain,chain_2,positions,marginal,radius=5.0,skip_dna=True):
    ...
    #define DNA bound nucleotides
    testing_positions=set([x for x in range(int(positions[0])-marginal,int(positions[1])+marginal)])
    list_of_atoms=[]
    for chain in structure_complex:
        if chain.id=="Y" or chain.id=="X":
           list_of_atoms.extend([x for x in chain.get_atoms()])
    neighbors = NeighborSearch(list_of_atoms,bucket_size=10)
    contacts  = neighbors.search_all(5.0,level='R')
    dna_residues=set()
    for res_1,res_2 in contacts:
              rchain_1 = res_1.get_parent()
              rchain_2 = res_2.get_parent()
              if rchain_1.id == "X" and int(res_1.id[1]) in testing_positions:
                 dna_residues.add(res_1)
                 if rchain_2.id == "Y": dna_residues.add(res_2)
              if rchain_2.id == "X" and int(res_2.id[1]) in testing_positions:
                 dna_residues.add(res_2)
                 if rchain_1.id == "Y":dna_residues.add(res_1)
    #print(dna_residues)
    ...
    #backbone_atoms = ["CA"]
    backbone_atoms = ["CA", "P"]
    #backbone_atoms = ["CA","CB","C","N","O","P","OP1","OP2","C4'","C5'","O3'"]
    #backbone_atoms = ["CA","CB","C","P"]
    #backbone_atoms = ["CA","CB","C","N","P"]

    if not skip_dna:
        max_contact_dimer   =150
        max_contact         =50
        max_pc_contact_dimer=10
        max_pc_contact      =10
        max_bb_contact_dimer   = 20
        max_bb_contact         = 5
        max_bb_pc_contact_dimer= 5
        max_bb_pc_contact      = 2
    else:
        max_contact_dimer   =50
        max_contact         =10
        max_pc_contact_dimer= 10
        max_pc_contact      = 1
        max_bb_contact_dimer   =2
        max_bb_contact         =1
        max_bb_pc_contact_dimer=2 
        max_bb_pc_contact      =1
    ...
    if len(structure_complex) == 2:
        ...
        for chain in structure_complex:
          ...
          contacts  = neighbors.search_all(radius,level='R')
          print("Total of contacts ",len(contacts))
          if chain.id=="X" and skip_dna:continue
          if chain.id=="Y" and skip_dna:continue
          for res_1,res_2 in contacts:
              rchain_1 = res_1.get_parent()
              rchain_2 = res_2.get_parent()
              if rchain_1.id == "X" and res_1 in dna_residues: continue
              if rchain_2.id == "X" and res_2 in dna_residues: continue
              if rchain_1.id == "Y" and res_1 in dna_residues: continue
              if rchain_2.id == "Y" and res_2 in dna_residues: continue
    ...
    if (chain.id,chain_2) not in paired_chains and (chain_2,chain.id) not in paired_chains:
        ...
        #if  max(bb_clash_percentage_structure) > 0 or max(bb_clash_percentage_model) > 0  or  float(bb_clash_num) > 0 :
        #  if  max(clash_percentage_structure) > 0 or max(clash_percentage_model) > 0 or  float(clash_num) > 0  :
        if  max(bb_clash_percentage_structure) > max_bb_pc_contact or max(bb_clash_percentage_model) > max_bb_pc_contact  or  float(bb_clash_num) > max_bb_contact :
          if  max(clash_percentage_structure) > max_pc_contact or max(clash_percentage_model) > max_pc_contact  or  float(clash_num) >  max_contact :
            for chain_1,chain_2  in contact_chains:
               if chain_1 == "X" and skip_dna:continue
               if chain_1 == "Y" and skip_dna:continue
        ...
        #if  max(bb_clash_percentage_structure) > 0 or max(bb_clash_percentage_model) > 0  or  float(bb_clash_num) > 0 :
        #  if  max(clash_percentage_structure) >  0 or max(clash_percentage_model) >  0 or  float(clash_num) > 0  :
        if  max(bb_clash_percentage_structure) > max_bb_pc_contact_dimer or max(bb_clash_percentage_model) >  max_bb_pc_contact_dimer or  float(bb_clash_num) > max_bb_contact_dimer :
          if  max(clash_percentage_structure) > max_pc_contact_dimer or max(clash_percentage_model) > max_pc_contact_dimer or  float(clash_num) > max_contact_dimer  :
            for chain_1,chain_2  in contact_chains:
               if chain_1 == "X" and skip_dna:continue
               if chain_1 == "Y" and skip_dna:continue
               incompatible.setdefault(chain_1,set()).add(chain_2)
...
def build_complex(structure_1, file_2, namemap, offset, marginal,radius=5.0,skip_dna=True): 
    ...
    try:
      if check_clash(structure_1, moved_chain,letter,positions,marginal,radius,skip_dna):
...
def recursive_build(structure, start_chain_ids, chainlist, complexname, args, namemap,output_fragment,radius=5.0,skip_dna=True):
    ...
    # RECURSIVE CASE. - When there are more chains to add in chainlist.
        ...
        for to_add in chainlist:
            ...
            clashed=False
            if not skip:
               clashed, newcomplex = build_complex(newcomplex, to_add, namemap,offset,marginal,radius,skip_dna)
            newlist = chainlist[:]
            newlist.remove(to_add)

            if clashed or skip:
                dummy_name=recursive_build(newcomplex, start_chain_ids, newlist, complexname, args, namemap,output_fragment,radius,skip_dna)
            else:
                print("\t--RECURSIVE STEP: chains on model "+complexname[4:-4]+" add new "+letter)
                dummy_name=recursive_build(newcomplex, start_chain_ids, newlist, complexname[:-4] + "_"+letter + ".pdb", args, namemap,output_fragment,radius,skip_dna)
...
if __name__ == "__main__": # Main part of the program.
    ...
    for startfile in dna_files:
      ...
      if end is None:
         ...
         output_fragment="fragment_1-100"
      else:
      ...
      dummy_name=recursive_build(complex_pdb, start_chain_ids, filelist, complexname, args, namemap,output_fragment,radius,skip_dna)
