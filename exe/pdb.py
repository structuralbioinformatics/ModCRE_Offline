...
# Import my modules #
from ModCRElib.beans import clean
from ModCRElib.builder import split
from ModCRElib.structure.contacts import contacts,interface,triads
from ModCRElib.structure.dna import x3dna,helices,modify_dna
from ModCRElib.structure.protein import dssp,tmalign, model_protein
from ModCRElib.structure.threading import threader
from ModCRElib.sequence import blast, nr, hmmer, homologs, folds
...
def parse_options():
    ...
    parser.add_option("-m", "--modify_DNA", default=False, action="store_true", dest="modify_dna", help="Modify the DNA structure by extending 10 nucleotides in both sides 5-prima 3-prima with AAA... to calculate the PWM (default = False)", metavar="{boolean}")
...
if __name__ == "__main__":
      ...
      if options.parallel:
        ...
        if options.family_cisbp_file is not None:
          ...
          for pdb_fasta in os.listdir( os.path.abspath(os.path.join(options.output_dir, "split"))):
            ...
            if pdb_split not in tfs_fam:continue
            ...
            if pdb_split in tfs_fam:
             if tfs_fam[pdb_split]=="Unknown" or tfs_fam[pdb_split]=="TBP":
                tfs_fam_set=set()
                try:
                 ...
                 exit(0)
                for fam in families:
                 if fam.upper() in family_dbd: tfs_fam_set.update(family_dbd[fam.upper()])
                 if fam.upper() in family_name: tfs_fam_set.update(set([x[1] for x in  family_name[fam.upper()]]))
          ...
          functions.write(tfs_file, "#%s;%s;%s" %("pdb","chain","family"))
          for pair,families in tfs_fam_new.items():
               pdb,chain=pair
      ...
      if options.parallel:
        ...
        if not os.path.exists(database_file):
            ...
            for fasta_file in sorted(os.listdir(os.path.join(options.output_dir, "split"))):
                ...
                #Skip if not in TFs
                if options.verbose: sys.stdout.write("\t-- check sequence %s \n" % (fasta_file)) 
                ...
                if os.path.exists(triads_file):
                    ...
                    if options.verbose: sys.stdout.write("\t\t-- add sequence %s to database \n" % (fasta_file)) 
    ...
    if options.start_step <= 4:
        ...
        for triads_obj_file in triads_objects_files:
            ...
            if folds_file in folds_check_done:
    ...
    if options.start_step <= 6:
        ...
        if not os.path.exists(dimers_file):
         if options.parallel:
            ...
            print(("Families that dimerize: ",families_dimerize))
            # For each line... #
            for line in functions.parse_file(os.path.abspath(options.tfs_file)):
                if line.startswith("#"): continue
                ...
                try:
                 for test_family in  data[-1].split(","):
                  if test_family in families_dimerize:
                    print(("Check dimers of %s %s Family %s\n"%(data[0],data[1],test_family)))
                    pdb_chains_dimerize.add("%s_%s" % (data[0], data[1]))
                    if os.path.exists(os.path.join(options.output_dir, "triads", "%s_%s.txt" % (data[0], data[1]))):
                        pdb_dimerize.add(data[0])
                except:
                  continue
            print(("Dimers to test: ",pdb_dimerize))
            # For each PDB file... #
            if len(pdb_dimerize)>0:
              for pdb_name in sorted(pdb_dimerize):
                # Skip rejected PDB entries #
                for dimer in dimers.get_dimers(os.path.join(os.path.abspath(options.output_dir), "clean", "%s.pdb" % pdb_name), os.path.abspath(options.output_dir),os.path.abspath(options.tfs_file),True):
    ...
    if options.start_step <= 7:
        ...
        if not os.path.exists(families_file):
         ...
         if options.parallel:
            ...
            failed={}
            for pdb_chain,families in tfs.items():
            ...
            failed_list=[pdb for pdb in failed.keys()]
    ...
    if options.start_step <= 8:
       ...
       for pdb_file in sorted(os.listdir(os.path.join(options.output_dir, "split"))):
         ...
         try:
           ...
           #Modify the DNA if the option is selected
           if options.modify_dna:
              if options.verbose: sys.stdout.write("\t\textend the DNA by 10 nucleotides at each side...\n")
              try:
                original_file = os.path.join(os.path.abspath(options.dummy_dir), "pwm", pdb_chain + ".original.pdb" )
                if not os.path.exists(original_file): dummy_pdb_obj.write(original_file)
                extension    = int(config.get("Parameters", "extend_DNA_binding"))
                conformation = "B"
                nucleotide   = "A"
                modify_dna.modify_dna(original_file,dummy_file,conformation,extension,nucleotide,True,options.dummy_dir,options.verbose)
              except Exception as e:
                sys.stdout.write("\t\t\tFailed to extend due to %s ...\n" % (e))
