            ...
            if line[0] == kmer:
                ...
                if len(negatives) > 0:
                    ...
                    if line[0] not in kmers: 
                       continue
                    if abs(GC_content(line[0]) - gc) > 0.15 and options.gc_content: continue
                    elif kmers[line[0]] <= max_escore_negatives:
                        negative_kmers.append((tf_id, int(i), pdb_chain, line[0], start, end, int(fold), tfs[tf_id].get_family(), float(line[1]), 0))
        min_negatives = min(len(negative_kmers),options.randoms)
        # If k-mers for TF... #
        if len(positive_kmers) == 1 and len(negative_kmers) >= min_negatives:
            # Write positive k-mer #
            functions.write(pdb_general_file, "%s" % ";".join(map(str, positive_kmers[0])))
            # Randomly select negative k-mers #
            for random_negative_kmer in random.sample(negative_kmers, min_negatives):
        ...
        for line in functions.parse_file(os.path.join(options.output_dir, "results", "pdb.general.taylor", tf_id + "." + i + "." + pdb_chain + "." + str(start) + "-" + str(end) + "." + fold + ".txt")):
            ...
            if line[0] == kmer:
                ...
                if options.gc_content:
                   if abs(GC_content(line[0]) - gc) <= 0.15: negative_kmers.append((tf_id, int(i), pdb_chain, line[0], start, end, int(fold), tfs[tf_id].get_family(), float(line[1]), 0))
                else:
                   negative_kmers.append((tf_id, int(i), pdb_chain, line[0], start, end, int(fold), tfs[tf_id].get_family(), float(line[1]), 0))
            # If k-mers for TF... #
            #min_negatives = min(len(negative_kmers),options.randoms)
            #if len(positive_kmers) == 1 and len(negative_kmers) >= min_negatives: break
    ...
    for line in functions.parse_file(os.path.join(options.output_dir, "folds", "family.txt")):
        ...
        for line in functions.parse_file(os.path.join(options.pbm_dir, "motifs", tfs[tf_id]._motifs[int(i)] + ".txt")):
            ...
            try:
                ...
                pass
        # For each line... #
        if not os.path.exists(os.path.join(options.output_dir, "results", "pdb.family", tf_id + "." + i + "." + pdb_chain + "." + str(start) + "-" + str(end) + "." + fold + ".txt")): continue
    ...
    for line in functions.parse_file(os.path.join(options.output_dir, "folds", "family.txt")):
        ...
        if os.path.exists(negatives_file):
            ...
            for line in functions.parse_file(negatives_file):
                negatives.add(line)
        # For each line... #
        if not os.path.exists(os.path.join(options.output_dir, "results", "pdb.family.taylor", tf_id + "." + i + "." + pdb_chain + "." + str(start) + "-" + str(end) + "." + fold + ".txt")): continue
    ...
    for line in functions.parse_file(os.path.join(options.output_dir, "folds", "family.txt")):
        ...
        if os.path.exists(negatives_file):
            ...
            for line in functions.parse_file(negatives_file):
                negatives.add(line)
        # For each line... #
        if not os.path.exists(os.path.join(options.output_dir, "results", "pbm.family", tf_id + "." + i + "." + pdb_chain + "." + str(start) + "-" + str(end) + "." + fold + ".txt")): continue
    ...
    for line in functions.parse_file(os.path.join(options.output_dir, "folds", "family.txt")):
        ...
        if os.path.exists(negatives_file):
            ...
            for line in functions.parse_file(negatives_file):
                negatives.add(line)
        # For each line... #
        if not os.path.exists(os.path.join(options.output_dir, "results", "pbm.family.taylor", tf_id + "." + i + "." + pdb_chain + "." + str(start) + "-" + str(end) + "." + fold + ".txt")): continue
