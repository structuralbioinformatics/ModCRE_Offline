...
from ModCRElib.profile import profile as PROFILE
...
def classify_pdbs(pdb_files):
    ...
    while(len(classified)<len(class_monomer) or len(original_classified)<len(classified)):
      ...
      print(("CLASSIFY PDB: CLUSTERS = "+str(cluster)))
      for pdb_file in class_monomer:
        ...
        for number,values in cluster.items():
            ...
            if distance < 10 or ratio<0.2: 
                print(("CLASSIFY PDB: CHECK CLUSTER %d (D= %f R= %f) TO ADD PDB= %s (%d %d)"%(number,distance,ratio,pdb_file,int(a),int(b))))
        ...
        if len(cluster_to_add)>0:
          ...
          number=select[0]
          print(("CLASSIFY PDB:  SELECT BEST  Min_distance= %f MinRatio=%f CLUSTER %d ADD PDB %s"%(mindi,minra,number,pdb_file)))
          cluster[number].add((pdb_file,int(a),int(b),0,0))
          print(("CLASSIFY PDB RESULTS: Cluster= %s"%str( cluster[number] )))
          classified.append(pdb_file)
      ...
      if original_classified==classified and iteration>3:
        ...
        for pdb_file in class_monomer:
           ...
           b  = int(positions[2])
           number_of_clusters = number_of_clusters +1 
           print(("CLASSIFY PDB:  NEW CLUSTER %d ( %d %d )= %s"%(number_of_clusters,int(a),int(b),pdb_file)))
           cluster.setdefault(number_of_clusters,set()).add((pdb_file,int(a),int(b),0,0))
           classified.append(pdb_file)
           skip=True
           iteration=0
    ...
    while(len(classified)<len(class_dimer) or len(original_classified)<len(classified)):
      ...
      for pdb_file in class_dimer:
        ...
        if len(cluster2)<=0:
           ...
           continue
        cluster_to_add=[]
        for number,values in cluster2.items():
            x=np.array([ int(p) for name,p,q,r,s in values ])
            y=np.array([ int(q) for name,p,q,r,s in values ])
            ...
            if ( distance_1 < 10 and distance_2 < 10) or (ratio_1 < 0.2 and ratio_2 < 0.2):
                ...
                print(("CLASSIFY PDB: CHECK DIMER TO ADD CLUSTER %d D= %f R= %f pdb= %s (%d %d)"%(number,distance_1,ratio_1,pdb_file,int(a),int(b))))
        if len(cluster_to_add)>0:
          mindi=min([dd for num,dd,rr in cluster_to_add])
          selected_clusters=[(num,rr) for num,dd,rr in cluster_to_add if dd==mindi]
          minra=min([rr for num,rr in selected_clusters])
    ...
    for key,values in cluster2.items():
        number_of_clusters = number_of_clusters + 1
        cluster_all.setdefault(number_of_clusters,values)

    cluster_domain={}
    for number_of_cluster in cluster_all.keys():
    ...
    order = [nc for nc,position in sorted(list(cluster_domain.items()),key=lambda x: x[1][0])]
...
def calculate_protein_profiles(info_file,pdb_files,name_of_profiles,label,output_dir,input_folder,dna_file,thresholds, energy_profile,pdb_file,pbm_dir,pdb_dir,families,potential_file, radius,fragment_restrict, binding_restrict, split_potential,auto_mode,family_potentials,pbm_potentials,score_threshold,taylor_approach,pmf,bins,known,meme,reset, refine, dummy_dir,verbose,save,reuse,redofimo,complete,methylation=False): 
    ...
    start_time = d_time = 0
    while( iterate ):
        start_dna_sequences=True
        for dna_name,dna_sequence,single_dna_file in dna_sequences:
          if start_dna_sequences:
             dna_name_first = dna_name
             start_dna_sequences = False
          for pdb_file in pdb_files:
            # Define the output names for the protein profile
            output_pdb_name=pdb_file.rstrip(".pdb")
            if label is not None: output_pdb_name = output_pdb_name +"."+ label
            original_meme    = os.path.join(output_dir,output_pdb_name + "." + dna_name_first+".meme")
            original_pwm     = os.path.join(output_dir,output_pdb_name + "." + dna_name_first+".pwm")
            original_msa     = os.path.join(output_dir,output_pdb_name + "." + dna_name_first+".msa")
            output_pdb_name  = output_pdb_name + "." + dna_name
            output_file      = os.path.join(output_dir,output_pdb_name)
            if functions.fileExist(original_meme) and not functions.fileExist(output_file+".meme"):
               if verbose:print(("\t-- Found PWM %s and copy to %s"%(os.path.basename(original_meme),output_pdb_name+".meme")))
            if functions.fileExist(original_meme) and not functions.fileExist(output_file+".meme"): shutil.copy(original_pwm,output_file+".meme")
            if functions.fileExist(original_pwm)  and not functions.fileExist(output_file+".pwm") : shutil.copy(original_pwm,output_file+".pwm")
            if functions.fileExist(original_msa)  and not functions.fileExist(output_file+".msa") : shutil.copy(original_pwm,output_file+".msa")
            ...
            if functions.fileExist(output_file+".pickle"):
                   ...
                   if redofimo and functions.fileExist(output_file+".meme"):
                      ...
                      profile_protein=pickle.load(inp)
                      inp.close()
                      ...
                      pickle.dump(profile_protein,out)
                      out.close()
                   info = open(info_file,"r")
                   skip_adding=False
                   for line in info:
            ...
            if parallel:
               ...
               program=os.path.join(exe_path,"single_profile.py")
               ...
               if refine >0        : parameters = parameters + " --refine %d "%refine
...
def parse_options():
    ...
    parser.add_option("-i", action="store", type="string", dest="input_folder", help="Input folder with PDB files. Mandatory", metavar="INPUT_FOLDER")
    ...
    if options.input_folder is None or (options.pbm_dir is None and options.potential_file is None)  or options.pdb_dir is None:
...
if __name__ == "__main__":
    ...
    if not input_folder.startswith("/"): input_folder = os.path.abspath(options.input_folder)
    if options.output_name is not None: output_name = options.output_name
    else: output_name = os.path.basename(input_folder)
    label=options.label
    if label is not None: output_name = output_name +"."+ label
    ...
    dna_sequences=[]
    if dna_file is not None: dna_sequences=get_dna_sequences( input_folder, dna_file)
    ...
    for number_of_cluster in cluster.keys():
      ...
      if len(dna_sequences)>0:
         done=calculate_protein_profiles(info_file,pdb_files,name_of_profiles,label,output_dir,input_folder,dna_file,fimo_thresholds, energy_profile,pdb_file,pbm_dir,pdb_dir,families,potential_file, radius,fragment_restrict, binding_restrict, split_potential,auto_mode,family_potentials,pbm_potentials,score_threshold,taylor_approach,pmf,bins,known,meme,reset, refine, dummy_dir,verbose,save,reuse,redofimo,complete,methylation) 
      ...
      for dna_name,dna_sequence,dummy_dna_file in dna_sequences:
        ...
        if dna_name not in name_of_profiles_per_dna:continue
        ...
        for name in name_of_profiles_per_dna.get(dna_name):
          ...
          profile_protein = pickle.load(out)
          out.close()
          ...
          for name_one in name_of_profiles_per_dna.get(dna_name_one):
           for dna_name_two,dna_sequence_two,dummy_dna_file_two in dna_sequences:
            ...
            for name_two in name_of_profiles_per_dna.get(dna_name_two):
                ...
                profile_protein_one = pickle.load(out)
                out.close()
                ...
                profile_protein_two = pickle.load(out)
                out.close()
