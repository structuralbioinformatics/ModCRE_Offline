...
def bijection_sequence_to_sequence(sequences_a,sequences_b,dummy_dir="tmp",homodimer=False):
    ...
    if len(list(sequences_a.keys()))>len(list(sequences_b.keys())):
    ...
    pairing={}
    used=set()
    for name_main,sequence_main in main_set.items():
       score={}
       if  len(used) == len(list(second_set.keys())) and not homodimer:
           pairing.setdefault(name_main,None)
           continue
       for  name_second,sequence_second in second_set.items():
       ...
       maximum_id = max([sc[0] for name,sc in score.items()]) 
       maximum_length=0
...
def renumber_pdb(pdb_file,sequences,dummy_dir="/tmp"):
    ...
    for chain_id,chain_seq in sequences.items():
       ...
       for residue in pdb_chain.aminoacids:
          ...
          if pair not in mapping: continue
...
def parse_options():
    ...
    parser = optparse.OptionParser("python model_protein.py -i input_file -p pdb_dir [--dummy=dummy_dir --n-model=n_model --n-total=n_total -o output_dir] [-a -d -f -m -e -r resolution_file -s --dimer --monomer --unbound_fragments --unrestrictive] [ --threading --structure ] ")
    ...
    group = optparse.OptionGroup(parser, "Threading options", "Provide a threading file as input (i.e from threader.py) or a PDB structure. Renumerate, all and full mode options can not be applied")
    ...
    if options.input_file is None  or (options.pdb_dir is None and not options.structure):
...
def get_pir_alignments(query_file, pdb_dir, full_mode=False,  filter_twilight_zone_hits=False, resolutions=None, select_mode=False, dummy_dir="/tmp/", original_sequence=None, 
    ...
    for hit in hits[0]:
        ...
        if thread_obj is None:
           ...
           if os.path.exists(os.path.join(pdb_dir,  "%s.pdb" % hit[0])): pdb_obj = PDB(os.path.join(pdb_dir,   "%s.pdb" % hit[0]))
           else: pdb_obj = PDB(os.path.join(pdb_dir, "split",  "%s.pdb" % hit[0]))
        # If TF dimerizes... #
        if hit[0] in dimers:
...
def filter_blast_hits_by_interface(query_file, blast_obj, pdb_dir, filter_twilight_zone_hits=False, dummy_dir="/tmp/", no_restrict=False,best_mode=False):
    ...
    for hit_obj in blast_obj.get_hits(tz_parameter=tz_parameter, tz_type=tz_type):
        ...
        hit_positions = list(range(hit_start - 1, hit_end))
...
def exec_matcher(A, B):
    ...
    try:
        ...
        # For each line... #
        count=0
        for line_coded in process.stdout.readlines():
...
def get_sequence_to_crystal_correlations(pdb_obj, pdb_chain, gapped=True):
    ...
    for protein_chain_obj in pdb_obj.proteins:
        ...
        sequence_positions = list(range(len(crystal_sequence)))
...
def get_protein_models(pir_alignment_obj, pdb_dir,  n=1, dummy_dir="/tmp/", verbose=False, optimization=False):
    ...
    if os.path.exists(os.path.join(pdb_dir, "clean", pir_alignment_obj.get_structure_name() + ".pdb")) :
      ...
      # Create dummy PDB file  and PIR_FILE#
      dummy_pdb_obj.write(dummy_file, force=True)
      pir_alignment_obj.write(pir_file)
    else:
      dummy_file = "template" + ".pdb"
      for pdb_chain_obj in sorted(PDB(os.path.join(pdb_dir,pir_alignment_obj.get_structure_name() + ".pdb")).chains, key=lambda x: x.chain):
        if pdb_chain_obj.chaintype == "P":
           if pdb_chain_obj.chain in pir_alignment_obj._structure_chains:
              if verbose: sys.stdout.write("\t\t-- add protein chain %s \n"%( pir_alignment_obj.get_structure_name() + "_" + pdb_chain_obj.chain))
              dummy_pdb_obj.add_chain(pdb_chain_obj)
      for pdb_chain_obj in sorted(PDB(os.path.join(pdb_dir,pir_alignment_obj.get_structure_name() + ".pdb")).chains, key=lambda x: x.chain):
        if pdb_chain_obj.chaintype == "N":
           if pdb_chain_obj.chain in pir_alignment_obj._structure_chains:
              if verbose: sys.stdout.write("\t\t-- add DNA/RNA chain %s \n"%( pir_alignment_obj.get_structure_name() + "_" + pdb_chain_obj.chain))
              dummy_pdb_obj.add_chain(pdb_chain_obj)
      # Sort PDB chains #
      dummy_pdb_obj.chains.sort(key=lambda x: x.chain)
      # Create dummy PDB file and PIR FILE#
      dummy_pdb_obj.write(dummy_file, force=True)
      pir_alignment_obj.add_structure_name("template")
      pir_alignment_obj.write(pir_file)
    # Execute modeller #
    flags=" "
...
def get_non_overlapping_pir_alignments(alignments_list):
    ...
    for pir_alignment_obj in alignments_list:
        ...
        residues = list(range(pir_alignment_obj._sequence_residues[0], pir_alignment_obj._sequence_residues[1] + 1))
        # For each PIR alignment object... #
        for non_overlapping_pir_alignment_obj in non_overlapping_alignments_list:
            # Initialize #
            non_overlapping_residues = list(range(non_overlapping_pir_alignment_obj._sequence_residues[0], non_overlapping_pir_alignment_obj._sequence_residues[1] + 1))
            # Get intersection #
...
def  get_pir_alignments_from_pdb(input_file):
    ...
    pdb_dir    = os.path.dirname(input_file)
    ...
    try: 
           pir_alignment_obj = PirAlignment()
           pir_alignment_obj.add_hit_name(hit_ID)
           pir_alignment_obj.add_sequence_name("query")
           pir_alignment_obj.add_structure_name(pdb_name)
           pir_alignment_obj.set_identity(identity)
           pir_alignment_obj.set_similarity(similarity)
           start=None
           has_protein=False
           has_dna=False
           p=0
           for pdb_chain_obj in sorted(pdb_obj.chains, key=lambda x: x.chain):
             if pdb_chain_obj.chaintype == "P":
               ...
               pir_alignment_obj.add_structure_chain(pdb_chain_obj.chain)
               pir_alignment_obj.add_structure_residue(pdb_chain_obj._all_residues[0].number)
               if start is None: start = str(pdb_chain_obj._all_residues[0].number)+pdb_chain_obj.chain
               pir_alignment_obj.add_structure_residue(pdb_chain_obj._all_residues[-1].number)
               end = str(pdb_chain_obj._all_residues[-1].number)+pdb_chain_obj.chain
               pir_alignment_obj.add_structure_alignment(pdb_chain_obj.protein_sequence)
               pir_alignment_obj.add_sequence_residue(1)
               pir_alignment_obj.add_sequence_residue(len(pdb_chain_obj.protein_sequence))
               pir_alignment_obj.add_sequence_alignment(pdb_chain_obj.protein_sequence)
               pir_alignment_obj.add_main_order(p)
               p=p+1
             if pdb_chain_obj.chaintype == "N":
               has_dna=True
               pir_alignment_obj.add_structure_chain(pdb_chain_obj.chain)
               ...
               pir_alignment_obj.add_structure_alignment("." * len(pdb_chain_obj.nucleotides))
               pir_alignment_obj.add_sequence_residue(1)
               pir_alignment_obj.add_sequence_residue(len(pdb_chain_obj.nucleotides))
               pir_alignment_obj.add_sequence_alignment("." * len(pdb_chain_obj.nucleotides))
           if not has_protein or not has_dna:
              sys.stdout.write("ERROR when getting PIR alignments\n")
              sys.stdout.write("ERROR: PDB INCOMPLETE\n")
              info=open(info_file,"a")
              info.write("%s\tFAIL\t%s\n"%(os.path.basename(input_file).rstrip(".fa"),socket.gethostname()))
              info.close()
              exit(0)
           pir_alignment_obj.add_domain(start+ ":" + end)
           pir_alignments_list.append(pir_alignment_obj) 
    except Exception as e:
           raise(e)
    return pir_alignments_list
...
def get_kmer_thread_from_threads(threading_files):
    ...
    try:
        for thr_file in threading_files:
            thread_object   = threader.Threaded(threading_file=thr_file)
            if kmers_thread is None:
               kmers_thread = thread_object.get_kmers_fixed()
               done=True
            else:
               if kmers_thread != thread_object.get_kmers_fixed(): done=False
    except Exception as e:
        raise("fail to get kmers_thread from threads")
    if done:
       return kmers_thread
    else:
       return None
...
def get_pir_alignments_from_threads(threading_files,pdb_dir,hit_name_pdb,label_chain):
    ...
    try:
         ...
         hit_complex=hit_name_pdb+"_"+label_chain
         if not os.path.exists(os.path.join(pdb_dir,hit_complex+".pdb")):
            pdb_dummy=PDB()
            dna_chains=False
            has_dna=False
            for thr_file in threading_files:
                ...
                pdb_obj_thr     = PDB(os.path.join(pdb_dir,hit_ID+".pdb"))
                has_protein     = False
                for chain_obj in sorted(pdb_obj_thr.chains, key=lambda x: x.chain):
                      if not dna_chains and chain_obj.chaintype == "N":
                         has_dna=True
                         pdb_dummy.add_chain(chain_obj)
                      if chain_obj.chaintype == "P" and thread_object.get_pdb_chain()==chain_obj.chain:
                         has_protein=True
                         pdb_dummy.add_chain(chain_obj)
                if not has_dna and not dna_chains: sys.stdout.write("ERROR: PDB %s INCOMPLETE, no DNA\n"%(hit_ID))
                if has_dna and not dna_chains:dna_chains=True
                if not has_protein: sys.stdout.write("ERROR: PDB %s INCOMPLETE, no PROTEIN\n"%(hit_ID))
            pdb_obj=PDB()
            for chain_obj in sorted(pdb_dummy.chains, key=lambda x: x.chain):
                pdb_obj.add_chain(chain_obj)
            pdb_obj.write(os.path.join(pdb_dir,hit_complex+".pdb"))
         else:
            pdb_obj=PDB(os.path.join(pdb_dir,hit_complex+".pdb"))
         #Assign alignments per chain
         thread_alignments={}
         for thr_file in threading_files:
             thread_object   = threader.Threaded(threading_file=thr_file)
             chain_id        = thread_object.get_pdb_chain()
             query_alignment = thread_object.get_alignment("query")
             hit_alignment   = thread_object.get_alignment("pdb")
             for chain_obj in sorted(pdb_obj.chains, key=lambda x: x.chain):
                 if chain_obj.chaintype != "P": continue
                 if chain_obj.chain == chain_id:
                     if chain_obj.protein_sequence == hit_alignment.replace("-",""):
                        thread_alignments.setdefault(chain_id,(hit_alignment,query_alignment))
                     else:
                        thread_alignments.setdefault(chain_id,None)
         #Make the pir alignments
         identity=100
         similarity=100
         pir_alignment_obj = PirAlignment()
         pir_alignment_obj.add_hit_name(hit_complex)
         pir_alignment_obj.add_sequence_name("query")
         pir_alignment_obj.add_structure_name(hit_complex)
         pir_alignment_obj.set_identity(identity)
         pir_alignment_obj.set_similarity(similarity)
         start=None
         has_protein=False
         ...
         for pdb_chain_obj in sorted(pdb_obj.chains, key=lambda x: x.chain):
             if pdb_chain_obj.chaintype == "P":
               has_protein=True
               chain_id   =pdb_chain_obj.chain
               pir_alignment_obj.add_structure_chain(pdb_chain_obj.chain)
               pir_alignment_obj.add_structure_residue(pdb_chain_obj._all_residues[0].number)
               if start is None: start = str(pdb_chain_obj._all_residues[0].number)+pdb_chain_obj.chain
               pir_alignment_obj.add_structure_residue(pdb_chain_obj._all_residues[-1].number)
               end = str(pdb_chain_obj._all_residues[-1].number)+pdb_chain_obj.chain
               if thread_alignments[chain_id] is not None:
                  structure_alignment,query_alignment = thread_alignments[chain_id]
                  length_query = len(query_alignment) - len([x for x in query_alignment if x=="-"])
               else:
                  length_query=0
                  sys.stdout.write("ERROR: missing the correct sequence of chain %s"%chain_id)
                  raise("ERROR: missing the correct sequence of chain %s"%chain_id)
               pir_alignment_obj.add_structure_alignment(structure_alignment)
               pir_alignment_obj.add_sequence_residue(1)
               pir_alignment_obj.add_sequence_residue(length_query)
               pir_alignment_obj.add_sequence_alignment(query_alignment)
               pir_alignment_obj.add_main_order(p)
               p=p+1
             if pdb_chain_obj.chaintype == "N":
               has_dna=True
...
if __name__ == "__main__":
    ...
    if options.structure: pdb_dir=os.path.dirname(input_file)
    else: pdb_dir = options.pdb_dir
    ...
    if options.threading:
     full_mode=False
     try:
      #Check the type of input thread
      input_threading_file=input_file
      input_folder=os.path.dirname(input_threading_file)
      try:
        pdb_name = None
        thr_obj = threader.Threaded(threading_file=input_threading_file)
        thr_obj._check_parsing()
        pdb_name =  thr_obj.get_pdb_name()
        if options.verbose: sys.stdout.write("\t-- Read single threading file on PDB %s ...\n"%pdb_name)
        if pdb_name is not None: 
           ...
           if n>50:
             if options.verbose: sys.stdout.write("\t-- Check your threading file: TOO LARGE COMPLEX...\n")
           if options.verbose: sys.stdout.write("\t-- Read complex threading file...\n")
           thread_complex=True
      except:
        fi=open(input_threading_file,"r")
        n=len([x for x in fi])
      ...
      if not thread_complex:
        threading_files = []
        threading_files.append(input_file)
      #Check that we can generate the model template with all chains (if it is a macrocomplex) or if the thread uses ModCRE pdb folder 
      exist_pdbs=True
      label_chain=""
      hit_name_pdb=None
      threading_files_clean=[]
      for thr_file in threading_files:
           if  options.verbose: sys.stdout.write("\t-- Check threading file %s ...\n"%(os.path.basename(thr_file)))
           if os.path.exists(thr_file): 
               threading_files_clean.append(thr_file)
           elif os.path.exists(os.path.join(input_folder,os.path.basename(thr_file))):
               threading_files_clean.append(os.path.join(input_folder,os.path.basename(thr_file)))
           else:
               threading_files_clean.append(os.path.basename(thr_file))
           if  options.verbose: sys.stdout.write("\t\t-- Add file %s\n"%(threading_files_clean[-1]))
      threading_files=threading_files_clean
      for thr_file in threading_files:
           thread_object   = threader.Threaded(threading_file=thr_file)
           hit_ID          = thread_object.get_pdb_name()+"_"+thread_object.get_pdb_chain()
           if  options.verbose: sys.stdout.write("\t-- Check thread %s ...\n"%(hit_ID))
           if hit_name_pdb == None: 
               hit_name_pdb =  thread_object.get_pdb_name()
           else:
               if hit_name_pdb != thread_object.get_pdb_name(): exist_pdbs=False
           if  options.verbose: sys.stdout.write("\t\t-- Check template %s ...\n"%(os.path.join(input_folder,hit_ID+".pdb")))
           if not exist_pdbs or not  os.path.exists(os.path.join(input_folder,hit_ID+".pdb")): exist_pdbs=False
           label_chain += thread_object.get_pdb_chain()
      #If PDB templates are not in the same folder, use  ModCRE pdb folder
      if not  exist_pdbs and thread_complex:
        print("Error: missing PDB templates of monomers in folder %s"%(input_folder))
        exit(0)
      if not  exist_pdbs:
       try:
        # Reading the classic threading file with templates in ModCRE#
        thread_object   = threader.Threaded(threading_file=input_file)
        hit_ID          = thread_object.get_pdb_name()+"_"+thread_object.get_pdb_chain()
        query_alignment = thread_object.get_alignment("query")
        hit_alignment   = thread_object.get_alignment("pdb")
        ...
        try:
           identity   =int((get_identities(query_alignment, hit_alignment) * 100) / (float(aligned_residues)))
        except:
           sys.stdout.write("ERROR to calculate identity percentage")
           identity=0
        similarity =identity
        ...
        pir_alignments_list = get_pir_alignments(sequence_file, pdb_dir, full_mode, options.filter_twilight_zone_hits, resolutions, options.select_mode, dummy_dir, thread_obj=hit_object, \
               ...
               sys.stdout.write("ERROR when getting PIR alignments from a monomer thread\n")
      ...
      # PDB templates are in the same folder, use the templates to generate the alignemnt
        pdb_dir=input_folder
        try:
           pir_alignments_list = get_pir_alignments_from_threads(threading_files,pdb_dir,hit_name_pdb,label_chain)
           kmers_thread        = get_kmer_thread_from_threads(threading_files)
           if kmers_thread is None:
              sys.stdout.write("ERROR: different kmers for therad files")
        except Exception as e:
           sys.stdout.write("ERROR when getting PIR alignments from threads\n")
           sys.stdout.write("ERROR: %s\n"%e)
           info=open(info_file,"a")
           info.write("%s\tFAIL\t%s\n"%(os.path.basename(input_file).rstrip(".fa"),socket.gethostname()))
           info.close()
           ...
           info.close()
           exit(0)
    # remodel an input structure
    elif options.structure:
        full_mode=False
        renumerate=False
        try:
           pir_alignments_list = get_pir_alignments_from_pdb(input_file)
        except Exception as e:
           sys.stdout.write("ERROR when getting PIR alignments of a 3D structure\n")
           sys.stdout.write("ERROR: %s\n"%e)
           info=open(info_file,"a")
           info.write("%s\tFAIL\t%s\n"%(os.path.basename(input_file).rstrip(".fa"),socket.gethostname()))
           info.close()
        ...
        try:
           ...
           sys.stdout.write("ERROR when getting PIR alignments of BLAST\n")
           sys.stdout.write("ERROR: %s\n"%e)
           info=open(info_file,"a")
           info.write("%s\tFAIL\t%s\n"%(os.path.basename(input_file).rstrip(".fa"),socket.gethostname()))
           info.close()
    ...
    for domain in domain_d.keys():
        n_skip_template=0
        skip_template=False
        for pir_alignment_obj in domain_d[domain]:
            #Check number of templates
            if options.n_temp is not None:
               n_skip_template=n_skip_template+1
               if options.n_temp < n_skip_template: skip_template=True
            ...
            for j in range(len(pir_alignment_obj.get_structure_chains())):
                ...
                if re.search("^\.+$", sequence_alignment):
                    ...
                    identities.append(str(int(((get_identities(sequence_alignment, structure_alignment) * 100) / float(aligned_residues)) )))
                    coverage.append(str(int(((aligned_residues * 100) / float(get_sequence_residues(structure_alignment))) )))
            ...
            for i in range(1, n+1):
                if options.threading:
                      pdb_file = os.path.basename(input_file).rstrip("txt")+"pdb"
                else:
            ...
            if exists == False or options.force:
              try:
                ...
                for j in range(n):
                    ...
                    # Write model #
                    if options.threading:
                       pdb_filename = os.path.basename(input_file).rstrip("txt")+"pdb"
                    else:
                    ...
                    if interface_obj.get_interface_length() == 0 and not options.full_fragments_mode and not options.threading:
                        ...
                        continue
                    # If model DNA... #
                    if options.dna or options.threading:
                        # Initialize #
                        dummy_file = os.path.join(dummy_dir, "%s.pdb" % dummy_name )
                        # Write model #
                        pdb_obj.write(dummy_file, force=True)
                        if options.threading:
                           dna_sequence    = [dna_seq for dna_seq in kmers_thread.keys()][0]
                           interface_start =  int(kmers_thread[dna_sequence])
                           interface_range =  list(range(interface_start, interface_start + len(dna_sequence)))
                           ...
                           if interface_obj.get_interface_start() is not None:
                              interface_range =  list(range(interface_obj.get_interface_start(),interface_obj.get_interface_start()+len(dna_sequence)))
                              pdb_obj = model_dna.get_dna_model_pdb_obj(dummy_file, dna_sequence, x3dna_obj, interface_obj, interface_range, dummy_dir=dummy_dir)
                    # If resolutions file... #
                    if options.resolution_file is not None:
                        # Write summary #
                        ...
                        if options.renumerate and not options.structure :
                           ...
                           homodimer=False
                           if (len(list(sequences_input.keys()))==1 and len(list(sequences_modelled.keys()))>len(list(sequences_input.keys()))) and not options.monomer_mode: homodimer=True
