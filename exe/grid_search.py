...
def parse_options():
    ...
    parser.add_option("-p", "--parallel", default=False, action="store_true", dest="parallel", help="Run parallel jobs in cluster")
...
if __name__ == "__main__":
        ...
        for family in list(families.keys()):
            ...
            for tf_id, motif in sorted(families[family]):
                ...
                if options.all_templates:
                   if options.n_temp is None:
                      flag="--n-model=1"
                   else:
                      flag=" --all --n-temp %d "%(options.n_temp)
                else:
                   flag="--best"
                if options.parallel:
                    #print("python " + scripts_path + "/model_protein.py -i " + sequence_file + " -o " + os.path.join(options.output_dir, "models") + " -l " + label + " --p=" + os.path.abspath(options.pdb_dir) + " --dummy=" + options.dummy_dir + " --renumerate -v  -s " + flag)
                    # Submit to queue #
    ...
    if options.start_step <= 3:
        ...
        flag=" "
        if options.reduce_thresholds: flag=flag + " --reduce_thresholds "
        if options.split_potential is not None: flag = flag + " -s "+options.split_potential
        if options.pmf: flag = flag + " --pmf "
        # For each PDB file #
        for pdb_file in os.listdir(os.path.join(os.path.abspath(options.output_dir), "models")):
            if pdb_file.endswith(".pdb"):
                sys.stdout.flush()
                ...
                if options.parallel:
                    ...
                    functions.submit_command_to_queue("%s %s -v -p %s -d %s -x %s -c %s -t %s  %s --pdb=%s --pbm=%s -o %s" % ("python", os.path.join(scripts_path, "grid_search_get_pwms.py"), pdb_path, dssp_file, x3dna_file, contacts_file, triads_file,flag, os.path.abspath(options.pdb_dir), os.path.abspath(options.pbm_dir), os.path.abspath(options.output_dir)), None, int(config.get("Cluster", "max_jobs_in_queue")), os.path.join(scripts_path, config.get("Cluster", "command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
                else:
                    os.system("%s %s -v -p %s -d %s -x %s -c %s -t %s %s --pdb=%s --pbm=%s -o %s" % ("python", os.path.join(scripts_path, "grid_search_get_pwms.py"), pdb_path, dssp_file, x3dna_file, contacts_file, triads_file, flag, os.path.abspath(options.pdb_dir), os.path.abspath(options.pbm_dir), options.output_dir))
    ...
    if options.start_step <= 5:
        ...
        for modcre_pwm_file in os.listdir(os.path.join(os.path.abspath(options.output_dir), "pwms")):
            ...
            if modcre_pwm_file.endswith(".meme.s"):
                ...
                pdb_tmp   = template.split("_")[0]
                chain_tmp = template.split("_")[1]
                ...
                #selected thresholds to speed up
                if int(threshold) not in selected_thresholds: continue
                if options.n_temp is not None:
                   pwm_name = tf_id + "_mL_" + motif + "_mL_" + family_tf + "_mL_" + potential + "_mL_" + data + "_mL_" + dist + "_mL_" + threshold
                   templates.setdefault(pwm_name,set()).add(template)
                   if len(templates[pwm_name]) > max_templates: continue
                pdb_chain = modcre_pwm.split("_mL_")[8]
                if chain_tmp != pdb_chain:
                   if options.verbose:sys.stdout.write("\t\t\t-- Skip different chains %s - %s \n"%(template,pdb_chain))
                   continue
                ...
                if options.parallel:
                  ...
                  if not os.path.isfile(tomtom_file) and not os.path.exists(tomtom_file+".gz"):
                    if modcre_pwm_file != None and tomtom_file != None:
                       ...
                       if options.verbose:sys.stdout.write("\t\t\t-- Add job %s %s  -d %s -i %s -o %s --dummy=%s"%(python,os.path.join(scripts_path,"tomtom.py"),cisbp_pwm,pwm_file,tomtom_file,os.path.abspath(options.dummy_dir)))
        ...
        if options.parallel and len(jobs)>0:
                  ...
                  for n in range(0,n_run):
                   sh_file=os.path.abspath("p_%d.sh"%(n))
                   if options.verbose:sys.stdout.write("\t-- submit %s\n"%(sh_file))
                   functions.submit_command_to_queue("sh " + sh_file,None, int(config.get("Cluster", "max_jobs_in_queue")), os.path.join(scripts_path, config.get("Cluster", "command_queue")),tmp_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
                   sys.stdout.flush()
    ...
    if options.start_step <= 6:
        ...
        for tfile in os.listdir(os.path.join(os.path.abspath(options.output_dir), "tomtom")):
          sys.stdout.flush()
          if tfile.endswith("txt.gz"): gz=True
          else: gz=False
          if tfile.endswith("txt") or  tfile.endswith("txt.gz"):
           try:
            if gz:
                tomtom_obj = tomtom.Tomtom(gzip.open(os.path.join(os.path.abspath(options.output_dir), "tomtom", tfile), "r").readlines())
            else:
            ...
            dist = distance   = tfile.split("_mL_")[6]
            ...
            if nbatch >= 10000:
                ...
                try:
                   pickle.dump(tomtom_lists,fdump_file_batch)
    ...
    if options.start_step <= 7:
        ...
        for tfile in os.listdir(os.path.join(os.path.abspath(options.output_dir), "results")):
            ...
            if tfile.startswith("grid_search_df"):
               ...
               tomtom_batch=pickle.load( open(os.path.join(os.path.abspath(options.output_dir), "results", tfile),"rb") )
       ...
       10 -> rank
        ...
        if "general" in min_results_dict:
    ...
    if options.start_step <= 8:
        ...
        topr_parameters={}
        top_parameters={}
        for family in best_parameters.keys():
        ...
        for family,parameters in selected.items():
            data          = parameters[0]
