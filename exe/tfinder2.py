...
def parse_options():
    ...
    parser.add_option("-p", action="store", type="string", dest="proteins_file", help="Proteins file (from CIS-BP; i.e. cisbp_1.02.proteins.sql)", metavar="{filename}")
    parser.add_option("--pdb", action="store", type="string", dest="pdb_dir", help="PDB directory (i.e. ./pdb/all)", metavar="{directory}")
    parser.add_option("--map", action="store", type="string", dest="map_file", help="ID mapping file with PDB_code vs UniprotID (download from UniProtKB; i.e. idmapping.dat)", metavar="{filename}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(pdb_dict_file):
        ...
        for pdb_file in os.listdir(os.path.abspath(options.pdb_dir)):
            # For each line... #
            gz=False
            if pdb_file.endswith(".gz") or pdb_file.endswith(".Z"): gz=True
            for line in functions.parse_file(os.path.join(os.path.abspath(options.pdb_dir), pdb_file),gz):
        ...
        for pdb_name, pdb_chain in pdb.keys():
    ...
    for pdb_file in os.listdir(os.path.abspath(options.pdb_dir)):
      if pdb_file[-3:] == "pdb" or  pdb_file[-4:] == "pdb1":
        ...
        print("PDB IN FOLDER",pdb_name)
        if pdb_name.lower() not in set([key.lower() for key,chain in pdball]):
               print("PDB UNDEFINED CHAIN",pdb_name)
    ...
    # Skip if dict file already exists #
    if not os.path.exists(cisbp_dict_file) or not os.path.exists(cisbp_dict_full):
        # Initialize
        species = {}
        families = {}
        # For each line... #
        ...
        print("Total TFs with known specie and protein sequence",len([x for x in cisbp.keys()]))
        print("Total missing proteins",len([x for x in missing_p.keys()]))
        print("Total missing TF",len([x for x in missing_t.keys()]))
        for mp,mt in missing_p.items():
            print("MISSING PROTEIN",mp,mt)
        for mt,mp in missing_t.items():
    ...
    if not os.path.exists(uniprot_dict_file) or not os.path.exists(uniprot_dict_full):
        # Initialize #
        gz = False
    ...
    if not os.path.exists(tfs_large_dict_file): 
     ...
     while len(pdbinput) > 0 and n<10:
      print("Step %d: Searching TF among %d PDB files\n"%(n,len(pdbinput)))
      tf_new=set()
      retry=set()
      (tf_new,retry)=TFinderSelect.TFinder(pdbinput,tfs_cisbp)
      pdbinput=retry
      print("Try again with PDBs",len(pdbinput))
      tfs_large.update(tf_new)
      n=n+1
     if n>=10:
      print("Reached more than 10 trial connections. Please check your internet provider\n")
