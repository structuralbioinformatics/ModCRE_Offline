...
from ModCRElib.sequence import SeqIO as SEQ
...
def table_tf_models(omdltab,  tfid, tfinfo, tf_mdl_cisbp_tomtom, tf_mdl_jaspar_tomtom, cisbp_jaspar_tomtom, verbose,tmp):
    ...
    for data in tfinfo:
        family,cisbp_fam,jaspar_id,tfmdl,motif_cisbp,motif_jaspar,motif_mdl=data
        if tfmdl not in tf_mdl_cisbp_tomtom: 
           if verbose:print(("\t--Skip model %s  vs CisBP Motif %s"%(tfmdl,motif_cisbp)))
           continue
        if tfmdl not in tf_mdl_jaspar_tomtom: 
           if verbose: print(("\t--Skip model %s  vs JASPAR Motif %s"%(tfmdl,motif_jaspar)))
           continue
        if motif_cisbp not in cisbp_jaspar_tomtom: 
           if verbose: print(("\t--Skip CisBP Motif %s  vs JASPAR Motif %s"%(motif_cisbp,motif_jaspar)))
           continue
...
def parse_options():
    ...
    parser.add_option("--species", default=None, action="store", dest="species", help="Species file to get ENTRY name codes as GENE_SPECIES (default is nOne and skips it use")
...
def main():
    ...
    if options.species is not None:
        species_file = open(options.species,"r")
        read = False
        for line in species_file:
            ...
            if read:
                ...
                if m:
                        if m.group(1) == "N":
                            species_code.setdefault(m.group(2),code)
        species_file.close()
        if verbose:
            print("SPECIES CODES:")
            for spec in list(species_code.keys()):
                print(("%s\t%s"%(spec,species_code.get(spec))))
    ...
    jaspar_ids=set([x.upper() for x in jaspar_data.keys()])
    ...
    for line in fl:
     ...
     tfid=data[0]
     tfname=data[1].upper()
     organism=" ".join(data[2].split("_"))
     if organism in species_code:
        code=species_code.get(organism)
        tfuid=tfname+"_"+code
        if verbose: print(("Add %s to TFID %s"%(tfuid,tfid)))
        cisbp_ids.setdefault(tfuid,tfid)
     #else:
     #   cisbp_ids.setdefault(tfname,tfid)
     tfgene=data[4].upper()
     cisbp_ids.setdefault(tfgene,tfid)
     if verbose: print(("Add %s to TFID %s"%(tfgene,tfid)))
    fl.close()
    ...
    for line in fl:
     ...
     for idmap_list in seq_codes:
      ...
      for idmap in idmap_list.split(";"):
        ...
        if idmap in jaspar_ids and not found:
           ...
           found=True
     if found:
      jaspar_codes.setdefault(jaspar_idm,seq_codes)
      for idmap_list in seq_codes:
       for idmap in idmap_list.split(";"):
        #if idmap.isalnum():
         idmap_jaspar.setdefault(idmap,jaspar_idm)
         all_codes.add(idmap)
     ...
     for idmap_list in seq_codes:
       for idmap in idmap_list.split(";"):
         #if idmap.isalnum():
           if idmap in cisbp_ids and not found:
               cisbp_idm=idmap
               found=True
     if found:
       cisbp_codes.setdefault(cisbp_idm,seq_codes)
       for idmap_list in seq_codes:
          for idmap in idmap_list.split(";"):
            if idmap.isalnum():
              idmap_cisbp.setdefault(idmap,cisbp_idm)
              all_codes.add(idmap)
    ...
    if verbose:
        print("JASPAR IDS")
        for jaspar_idm in list(jaspar_codes.keys()):
            print(("%s : \t%s"%(jaspar_idm,str(jaspar_codes.get(jaspar_idm)))))
        print("CISBP IDS")
        for cisbp_idm in list(cisbp_codes.keys()):
            print(("%s : \t%s"%(cisbp_idm,str(cisbp_codes.get(cisbp_idm)))))
    ...
    if verbose:
        print("JASPAR CORRESPONDENCE TO CISBP")
        for jaspar_id in list(jaspar_cisbp.keys()):
            print(("%s : \t%s"%(jaspar_id,str(jaspar_cisbp.get(jaspar_id)))))
        print("CISBP CORRESPONDENCE TO JASPAR")
        for cisbp_id in list(cisbp_jaspar.keys()):
            print(("%s : \t%s"%(cisbp_id,str(cisbp_jaspar.get(cisbp_id)))))
    ...
    for tf_file in list_of_tf_files:
      ...
      if tf_short not in cisbp_jaspar: continue 
    ...
    for cisbf_fam,classic in cisbp_to_classic_families.items():
         nf+=1
    ...
    if fileExist(cisbp_jaspar_pickle):
             ...
             cisbp_jaspar_tomtom = pickle.load(open(cisbp_jaspar_pickle,"rb"))
    else:
    ...
    total_table_mean=[]
    summary_table=[]
    for tfid,tfinfo in tfid_data.items():
      ...
      if fileExist(tfid_jaspar_pickle):
             ...
             tf_mdl_jaspar_tomtom = pickle.load(open(tfid_jaspar_pickle,"rb"))
      else:
             if options.parallel:
                if tfid_jaspar_pickle not in submitted:
                  program=os.path.join(exe_path,"compare_model_jaspar.py")
                  cluster_queue=None
      ...
      tf_mdl_cisbp_tomtom=pickle.load(open(tfid_cisbp_pickle,"rb"))
      ...
      try:
         ...
         print(("Failed on %s (INFO %s) due to %s"%(tfid,str(tfinfo),e)))
         continue
    ...
    for motifs,data_list in summary_per_family_motif.items():
        ...
        success_r_cisbp      =sum(data[:,3]>0.5)
        success_r_jaspar     =sum(data[:,2]>0.5)
        success_min_cisbp    =sum(data[:,3]>0)
        success_min_jaspar   =sum(data[:,2]>0)
    ...
    for cisbp_fam,data_list in summary_per_family.items():
        data_all=np.array(data_list)
        data=np.array(data_all[:,1:],dtype=float)
        tfids=data_all[:,0].tolist()
        number_of_tfs        =len(set(tfids))
