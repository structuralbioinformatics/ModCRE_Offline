...
def create_inputboxplot(input_name,label,plot_step,fileroot,selected_score,verbose):
    ...
    for perc in range(100,plot_step,-plot_step):
...
def create_mdlboxplot_global(tf,found_interval,plot_step,fileroot,selected_score,normalize,maximum,minimum,verbose):
    for tf_pair, scores in tf.items():
        for perc in range(plot_step,100,plot_step):
            middle=int(perc+plot_step/2)
            if middle not in found_interval: continue
...
def create_average_models(tf_models):
    ...
    tf_maxim_models={}
    tf_minim_models={}
    for tf_pair,scores in tf_models.items():
            tf_list_mdl_scores.setdefault(tf_pair[0],[]).append(scores)
    for tf_name_query, list_scores in tf_list_mdl_scores.items():
...
def create_tf_comparisons(outable,dmdltmt,pwm_out,pwm_db,tf_neighbours,tf_nameseq,tf_nameseq_rev,tf_has_motif,tf_families,tf_families_cisbp,verbose,tmp):
     ...
     for tf_name_query,tf_relatives in tf_neighbours.items(): 
        if tf_name_query not in tf_nameseq: continue
        tf_query=tf_nameseq.get(tf_name_query)
        if tf_query not in tf_has_motif:continue
        motif_query = tf_has_motif.get(tf_query)
     ...
     for tf_name_query,tf_relatives in tf_neighbours.items():  
      ...
      for tf_name_target,tf_id,tf_blast in tf_relatives:
          ...
          if tomtom_obj.has_hit(motif_query):
           ...
           tf_query_family="Unknown"
           tf_query_family_cisbp="Unknown"
           if tf_name_query_family in tf_families: tf_query_family="+".join(["_".join(str(x.strip()).split()) for x in tf_families.get(tf_name_query_family)])
           if tf_name_query_family in tf_families_cisbp: tf_query_family_cisbp="+".join(["_".join(str(x.strip()).split()) for x in tf_families_cisbp.get(tf_name_query_family)])
...
def calculate_maxmin(tf_compare,tf_models,tf_nameseq,tf_nameseq_rev,tf_families,set_of_families,selected_score):
      ...
      maximum_seq={}
      if "global" in seq_scores_in_family: maximum_seq.setdefault("global",max([float(x) for x in seq_scores_in_family.get("global")]))
      for family in set_of_families:
        if family in seq_scores_in_family:
          maximum_seq.setdefault(family,max([float(x) for x in seq_scores_in_family.get(family)]))

      minimum_seq={}
      if "global" in seq_scores_in_family: minimum_seq.setdefault("global",min([float(x) for x in seq_scores_in_family.get("global")]))
      for family in set_of_families:
        if family in seq_scores_in_family:
          minimum_seq.setdefault(family,min([float(x) for x in seq_scores_in_family.get(family)]))

      maximum_mdl={}
      if "global" in mdl_scores_in_family: maximum_mdl.setdefault("global",max([float(x) for x in mdl_scores_in_family.get("global")]))
      for family in set_of_families:
        if family in mdl_scores_in_family:
          maximum_mdl.setdefault(family,max([float(x) for x in mdl_scores_in_family.get(family)]))

      minimum_mdl={}
      if "global" in mdl_scores_in_family: minimum_mdl.setdefault("global",min([float(x) for x in mdl_scores_in_family.get("global")]))
      for family in set_of_families:
        if family in mdl_scores_in_family:
...
def calculate_enrichment_neighbours(outable,top,normalize,plot_step,number_of_motifs,verbose):
    ...
    tf_enrichment={}
    for tf_perc, list_of_hits in tf_list_of_hits_by_neighbour.items():
        tf_query,percentage_id=tf_perc
        group=Counter(list_of_hits)
        if tf_perc not in tf_number_of_neighbours: continue
        if tf_query in group and tf_number_of_neighbours.get(tf_perc)>0:
        ...
        for hit,k in sorted(iter(group.items()),key=lambda x:x[1],reverse=True):
            if count != int(k): 
                count=int(k)
                n    =n+1
            if hit == tf_query: 
        ...
        if e>0 and tf_query in group and tf_number_of_neighbours.get(tf_perc)>0:
        ...
        max_enrichment=max([x for x in group.values()])
        #Get the average of scores (p_value, e_value,q_value) with all the selected solutions in the enrichment set
        p_score=0.0
        e_score=0.0
        q_score=0.0
        if tf_perc in tf_pval_list_of_hits: p_score = sum([float(x) for x in tf_pval_list_of_hits.get(tf_perc)])/len(tf_pval_list_of_hits.get(tf_perc))
        if tf_perc in tf_eval_list_of_hits: e_score = sum([float(x) for x in tf_eval_list_of_hits.get(tf_perc)])/len(tf_eval_list_of_hits.get(tf_perc))
        if tf_perc in tf_qval_list_of_hits: q_score = sum([float(x) for x in tf_qval_list_of_hits.get(tf_perc)])/len(tf_qval_list_of_hits.get(tf_perc))
...
def calculate_enrichment(tf_models,tf_mdl_tomtom,tf_has_motif,tf_nameseq,top,normalize,number_of_motifs,verbose):
    ...
    for tf_query,list_of_models in tf_list_of_models.items():
    ...
    tf_enrichment={}
    for tf_name_query, list_of_hits in tf_list_of_hits_by_models.items():
        if tf_name_query not in tf_nameseq: continue
        tf_query=tf_nameseq.get(tf_name_query)
        if tf_query not in tf_has_motif: continue
        motif_query = tf_has_motif.get(tf_query)
        ...
        if motif_query in group and tf_number_of_models.get(tf_name_query)>0:
        ...
        #Find the minimum enrichment with p-value lower than the random selection (1/number_of_motifs): lower than this implies the enrichment is as good as random
        if e>0 and motif_query in group and tf_number_of_models.get(tf_name_query)>0:
        ...
        q_score=0.0
        ranking=0.0
        if tf_name_query in tf_pval_list_of_hits: p_score = sum([float(x) for x in tf_pval_list_of_hits.get(tf_name_query)])/len(tf_pval_list_of_hits.get(tf_name_query))
        if tf_name_query in tf_eval_list_of_hits: e_score = sum([float(x) for x in tf_eval_list_of_hits.get(tf_name_query)])/len(tf_eval_list_of_hits.get(tf_name_query))
        if tf_name_query in tf_qval_list_of_hits: q_score = sum([float(x) for x in tf_qval_list_of_hits.get(tf_name_query)])/len(tf_qval_list_of_hits.get(tf_name_query))
        if tf_name_query in tf_rank_list_of_hits: ranking = sum([float(x) for x in tf_rank_list_of_hits.get(tf_name_query)])/len(tf_rank_list_of_hits.get(tf_name_query))
        #store the data in a new dictionary named tf_enrichment
        if e<min_enrichment:
...
def main():
    ...
    mmseqs=config.get(os.path.join(src_path,config.get("Paths", "mmseqs"))
    ...
    for cis,fam in cisbp_families.items():
    ...
    for tf,fam in tf_families.items():
        fo.write("%s\t%s\n"%(tf, ";".join([str(x) for x in fam]) ))
    fo.close()
    fo=open("families_cisbp.txt","w")
    for tf,fam in tf_families_cisbp.items():
        fo.write("%s\t%s\n"%(tf, ";".join([str(x) for x in fam])))
    fo.close()
    ...
    if not fileExist(pwm_db) or not fileExist(fasta_file):
     ...
     for tf,motif in tf_has_motif.items():
       try:
        pwm_file = options.pwm_dir+"/"+motif
        tf_file  = options.pbm_dir+"/sequences/"+tf
        if fileExist(pwm_file) and fileExist(tf_file) and tf in tf_nameseq_rev:
...
#For each TF query in  tf_neighbours 
    ...
    for tf_name_query in tf_neighbours.keys():
      ...
      if fileExist(omdltab) and fileExist(omdltmt):
        ...
        tf_mdl_tomtom=pickle.load(open(omdltmt,"rb"))
      else:
        if options.parallel:
         if modelExist(tf_name_query,tf_nameseq,tf_nameseq_rev,tf_has_motif,tf_has_model,verbose):
            program=os.path.join(exe_path,"compare_tf_models_with_motifs.py")
            cluster_queue=None
      ...
      for key,value in tf_models_tf.items():
          tf_models.setdefault(key,value)
      #Calculate enrichment
      if enrichment:
         number_of_motifs=len([x for x in tf_neighbours.keys()])
         ...
         for key,value in tf_enrichment_tf.items():
...
#variables to Normalize scores
    set_of_families=set([clean_name("+".join(["_".join(str(x).split()) for x in fam])) for fam in tf_families.values()])
    if enrichment:
...
# Global
    input_name   =  outplot+"_global.dat"
    boxplot_name =  outplot.replace(".","_")+"_global"
...
# Families
    for fam in tf_families.values():
        family=clean_name("+".join(["_".join(str(x).split()) for x in fam]))
        input_name   =  outplot+"_"+family+".dat"
        boxplot_name =  outplot.replace(".","_")+"_"+family
...
# TPR curves
    if selected_score == 3 or selected_score == 5: 
       ...
       y=[]
       z=[]
       for middle in sorted([int(key) for key in tpr_mdl.keys()]):
           if middle in tpr_seq and middle in tpr_mdl:
       ...
       for family in set_of_families:
           ...
           if len([x for x in tpr_mdl.keys()]) > 0 and len([x for x in tpr_seq.keys()]) > 0:
       ...
       for middle in sorted([int(x) for x in tpr_lst_seq.keys()]):
