...
for tf_seq in os.listdir(tf_dir):
    if tf_seq.endswith("pdb"):
      ...
      skip=False
      #pmdl     = "_".join([x for x in tf_seq.rstrip(".pdb").split("_")[:-1]])
      #pmdl_num   tf_seq.rstrip(".pdb").split("_")[-1]
      pmdl      = tf_seq.rstrip(".pdb").split(":")[0]
      pmdl_num  = ":".join([x for x in tf_seq.rstrip(".pdb").split(":")[1:]])
      if os.path.exists(output_pwm) and os.path.exists(output_meme) and os.path.exists(output_msa):
         pwm_models.setdefault(pmdl,[]).append(pmdl_num)
         print("Use %s (Models of %s: %d)"%(output_pwm,pmdl,len(pwm_models[pmdl])))
         skip=True
      if pmdl in pwm_models:
         if len(pwm_models[pmdl]) > max_number_of_models: skip=True
      if not skip:
        pwm_models.setdefault(pmdl,[]).append(pmdl_num)
        if  config.get("Cluster", "cluster_queue") == "None": cluster_queue=None
        else: cluster_queue=config.get("Cluster", "cluster_queue")
        print("%s %s --radius=%f -i %s -o %s --pdb=%s --pbm=%s --auto -v --dummy=%s  --refine 2 " % ( os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "pwm_pbm.py"), radius, input_file, output_file, os.path.abspath(pdb_dir), os.path.abspath(pbm_dir) , os.path.abspath(dummy)))
        functions.submit_command_to_queue("%s %s --radius=%f -i %s -o %s --pdb=%s --pbm=%s --auto -v --dummy=%s  --refine 2 " % ( os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "pwm_pbm.py"), radius, input_file, output_file, os.path.abspath(pdb_dir), os.path.abspath(pbm_dir) , os.path.abspath(dummy) ) , cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),dummy,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
