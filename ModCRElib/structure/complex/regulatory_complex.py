...
from ModCRElib.beans.functions import parse_fasta_file
...
from SBILib.structure.contacts import Complex
...
class macro_complex(object):
       ...
       for fragment in list(self._complexes.keys()):
          ...
          for line in fo:
              if line.startswith("REMARK"):
                 ...
                 if protein not in sequences: continue
   ...
   def set_protein_inner_restrains_in_core(self,protein,dsize=None,sfactor=None,threshold=None,maxdist=None):
       ...
       for i in range(len(ca_pos)):
           ...
           for j in range(a,b):
             ...
             if pos_i in ca_xyz and pos_j in ca_xyz:
   ...
   def extend_protein_inner_restrains(self,protein,dsize=None,sfactor=None,threshold=None,maxdist=None):
       ...
       if protein in self._chain_correspondence_plus:
   ...
   def get_protein_by_pdb_and_chain(self,pdb_file,chain):
       ...
       if fragment_select is None or failed:
           if tuple((pdb_file,chain)) not in self._chain_correspondence_plus: return None
   ...
   def accept_addition(self,pdb_obj,protein_num,protein_chain,partner,partner_chain,restraints,condition):
       ...
       for pair in list(restraints.keys()):
   ...
   def add_topology_and_connect(self,number_of_addition,protein_num,pdb_file,protein_chain,partner,partner_chain,restraints,condition=None):
       ...
       for new_range in new_ranges:
           ...
           if "rigid_body" in self._topology: rb=len(self._topology.get("rigid_body")) + 1
           else: rb=1
       ...
       if protein_num in self._chain_correspondence_in_core and condition=="old":
       ...
       for gap in gaps:
           ...
           self._restraints.setdefault(tuple((pos_prot_1,pos_prot_2)),restraint)
       #Check that new PPI restraints are possible
       if partner in self._chain_correspondence_in_core and condition=="old" and add_ppi_restraints:
           if maximum_gap < self._domain_connection : 
   ...
   def add_interfaces(self,interface_1,interface_2,new_protein,partner_in_complex):
       ...
       for p,aas in interface_2.items():
           if p==protein_2:
   ...
   def add_ppi(self,pdb_file,protein_1,protein_2,sequence_1,sequence_2,condition=None,maximum=None):
       ...
       for interface in pdb_complex.PPInterfaces:
               ...
               protein_num_1 = None
               protein_num_2 = None
               if protein_chain_id_1 in protein_chain: protein_num_1      = protein_chain[protein_chain_id_1]
               if protein_chain_id_2 in protein_chain: protein_num_2      = protein_chain[protein_chain_id_2]
               #print("\t\t-- PPI "+protein_num_1+" with "+protein_num_2)
       ...
       for protein in set_1:
           ...
           if condition == "old":
             ...
             for p in set_2:
                 ...
                 if protein_1+":A" not in protein_chain:continue
                 if protein_2+":B" not in protein_chain:continue
       ...
       if condition == "old":
         if right_oriented:
          ...
          if add_ppi_restraints:
             print(("\t\t\t-- Add restrain contacts (%s %s) %d"%(protein_num,partner_in_complex,len([x for x in list(restraints_1.keys())]))))
             new_restraints  = new_restraints +len([x for x in list(restraints_1.keys())])
          ...
          if add_ppi_restraints:
             print(("\t\t\t-- Add restrain contacts (%s %s) %d"%(protein_num,partner_in_complex,len([x for x in list(restraints_2.keys())]))))
             new_restraints  = new_restraints +len([x for x in list(restraints_2.keys())])
   ...
   def contains(self,other):
       ...
       for p in proteins_b:
           for b in set_b[p]:
               ...
               a_rank = [a for a,y in sorted(iter(all_correspondence.items()), key=lambda x: x[1],reverse=True )]
       ...
       if self._restrict_equal:
         restraints_a  = set([x for x in list(self.get_restraints().keys())])
         orestraints_b = set([x for x in list(other.get_restraints().keys())])
         restraints_b  = set()
         for x,y in orestraints_b:
           protx,aax = x
           if protx in correspondence_b: 
               xx = correspondence_b[protx]
           else:
               continue
           proty,aay = y
           if proty in correspondence_b: 
       ...
       # Check conditions
       if  set(correspondence_b.keys()) == set(other.get_protein_chains()) and len(list(correspondence_a.keys())) == len(self.get_protein_chains()) and set([x for k,x in correspondence_a.items()]) == set(other.get_protein_chains()) and contain_restraints: 
              return True
       return False
   ...
   def set_topology_core(self):
      ...
      for protein in self._protein_chains:
          ...
          if "rigid_body" in data: rb=len(data.get("rigid_body")) + 1
          else: rb=1
           ...
           if molecule_name[i] not in self._dna_chains:
             ...
             if skip_protein:
               ...
               for pair in delete_restraints:
                 if pair in list(self._restraints.keys()):
       ...
       for c in self._dna_chains:
        ...
        for i in range(len(phos_pos[c])):
          ...
          for j in range(a,b):
             ...
             gen_j = tuple((c,j-i))
             if tuple((pos_i,pos_j)) in done:continue
             if pos_i in phos_xyz and pos_j in phos_xyz:
...
if __name__ == "__main__":
    ...
    for complex_id in complex_list:
      ...
      for tf_comb in data:
        ...
        if fragment_folder in complex_codes:
          idx=int(cmp_num)-1
    ...
    for tf_complex in tf_complex_list:
        ...
        for fragment in list(complex_files.keys()):
            for i in range(len(fragments)):
