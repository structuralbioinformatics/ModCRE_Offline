...
#import web modules
from ModCRElib.web import mp2d
from ModCRElib.web import md2p
from ModCRElib.web.lib.model.experiment import ExperimentFabric, Experiment
from ModCRElib.web.lib.src.filehelp import text2file,file2text
from ModCRElib.web.lib.src.JSONfile import readJSON, dumpJSON
...
def copy_experiment(exp0):
            ...
            if os.path.exists(exp0.main_dir):
                ...
                job_time = get_time()
                if "job_name" not in exp0.__dict__:
                    name = "NEW"
                else:
            ...
            for key in list(exp0.__dict__.keys()):
                try:
                  if key=="motifs":
                     ...
                     for k in range(0, len(exp0.__dict__[key])):
                       ...
                       for i in range(0, len(exp0.__dict__[key][k])):
                           ...
                           for key_motif in list(motif.keys()):
                             try:
...
def api_submit():
    ...
    if bottle.request.method == "POST":
        ...
        if bottle.request.forms.get('type') == "protein2dna":
            ...
            print(("\n\nbefore protein2dna the experiment dict has " + str(len(list(exp.__dict__.keys()))) + " keys\n"))
            print(("EXECUTE MP2D IN "+output_dir))
            exp.__dict__ = mp2d.protein_2_dna(exp.__dict__, output_dir, remote=True, verbose=True)
            print(("\n\nafter protein2dna the experiment dict has " + str(len(list(exp.__dict__.keys()))) + " keys\n"))
            exp.__dict__["job_name"] = name
        ...
        if bottle.request.forms.get('type') == "dna2protein":
            ...
            # Execute programs from the server-side #
            print(("\n\n\n\nbefore dna2protein the experiment dict has " + str(len(list(exp.__dict__.keys()))) + " keys\n\n\n\n"))
            exp.__dict__["show_models"] = False
            full_json_dict = md2p.dna_2_protein(exp.__dict__, output_dir,remote=True, verbose=True)
            print(("\n\n\n\nafter dna2protein the experiment dict has " + str(len(list(exp.__dict__.keys()))) + " keys\n\n\n\n"))
        ...
        if bottle.request.forms.get('type') == "dna2protein_modeling":
            ...
            dna_seq = bottle.request.forms.get('dna_seq')
            dna_str = bottle.request.forms.get('dna_str')
            if "dna_str" not in exp.__dict__:  exp.__dict__["dna_str"]=dna_str
            # Create input json #
...
def api_job():
    ...
    search = dict((k, bottle.request.query.getall(k)) for k in list(bottle.request.query.keys()))
...
def write_finish_job(job_id, job_time, name, job_type):
    ...
    if os.path.isfile(os.path.join(result_dir, job_id, job_id + "_full.json")):
        ...
        if "error_msg" in list(json_content.keys()):
        ...
        if len(list(json_content.keys())) == 0:
            # This means that there has been a error in this execution #
...
def update_jobs():
         ...
         for line in running:
            ...
            if os.path.isfile(os.path.join(result_dir, line_id, line_id + "_full.json")):
                ...
                try:
                    ...
                    # This means that there has been a error in this file (it may be an old file with wrong style inputs) #
                    print(("Error of input/output formats "+line_id))
                    error_msg="Incompleted, check input formats"
                ...
                if "error_msg" in list(json_content.keys()):
                    ...
                    continue
                elif len(list(json_content.keys())) == 0:
                    # This means that there has been a error in this execution #
                ...
                elif job_type == "protein2dna" and len(list(json_content.keys())) <= 15:
        ...
        for folder in os.listdir(result_dir):
            if os.path.isfile(os.path.join(result_dir, folder, folder + "_full.json")):
                ...
                try:
                  ...
                  job_id = folder
                  print(("SKIP JOB %s CANNOT READ JSON"%folder)) 
                  continue
                job_id = folder
                job_type=json_content["type"]
                if "job_name" in  list(json_content.keys()):
                    job_name=json_content["job_name"]
                else:
                    job_name=job_id
                if "job_time" in list(json_content.keys()):
                ...
                if "error_msg" in list(json_content.keys()):
                    ...
                    continue
        print("REORDERING ....")
        for epoch_time in sorted(list(finished_time.keys()),reverse=True):
    ...
    for dire in os.listdir(result_dir):
      try:
        time_difference = float(current_time) - float(os.path.getmtime(os.path.join(result_dir, dire)))
        if time_difference > float(2592000) and dire in list(failed_folders.keys()):
