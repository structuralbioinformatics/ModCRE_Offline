...
from ModCRElib.web.lib.src import JSONfile
from ModCRElib.profile import  profiler as PROFILER
...
def update_profiles(json_dict,output_dir,remote):
    ...
    if "input_pwms" in list(json_dict.keys()): input_pwms_data = handle_input_pwms(json_dict["input_pwms"])
...
def update_classified_pdbs(pdb_files):
    ...
    cluster_domain={}
    for number_of_cluster in list(cluster.keys()):
...
def get_cluster_fragments(clusters,fragment_list):
    ...
    groups_by_cluster={}
    for number_of_cluster in list(clusters.keys()):
    ...
    for clu in list(groups_by_cluster.keys()):
    ...
    if len(fragment_list)>0:
      for number_of_cluster in list(clusters.keys()):
        ...
        if data_set[1] is not None:
           ...
           for interval in list(groups_by_fragment.keys()):
    ...
    for ID in range(1, len(motif_list)+1):
        ...
        for mot in clu:
            ...
            if len(mot["domain"].split(":")) == 2:
                domain = list(range(int(mot["domain"].split(":")[0]), int(mot["domain"].split(":")[1])))
                xvals = set()
                for d in domain:
                    xvals.add(d)
                x_values = list(xvals)
            ...
            if len(mot["domain"].split(":")) == 3:
                ...
                x_values = list(range(min(range(int(dom1.split(":")[0]), int(dom1.split(":")[1]))), max(range(int(dom1.split(":")[0]), int(dom1.split(":")[1]))), 1))
...
def clean_up_profiles(json_dict,working_dna):
  ...
  if "binding_profiles" in json_dict:
      for number_of_cluster in list(json_dict["binding_profiles"].keys()):
        for key in list(json_dict["binding_profiles"][number_of_cluster].keys()):
            if key == "comparison": continue
            if not key in working_dna: 
                json_dict["binding_profiles"].pop(key, None)
        if "comparison" in json_dict["binding_profiles"][number_of_cluster]:
          for twokey in list(json_dict["binding_profiles"][number_of_cluster]["comparison"].keys()):
  ...
  # For single Motifs
  if  "motifs" in json_dict:
    for k in range(0, len(json_dict["motifs"])):
     for i in range(0, len(json_dict["motifs"][k])):
       if "binding_profiles" in json_dict["motifs"][k][i]:
        for key in list(json_dict["motifs"][k][i]["binding_profiles"].keys()):
            if key == "comparison": continue
            if not key in working_dna:
               json_dict["motifs"][k][i]["binding_profiles"].pop(key, None)
        if "comparison" in json_dict["motifs"][k][i]["binding_profiles"]:
          for twokey in list(json_dict["motifs"][k][i]["binding_profiles"]["comparison"].keys()):
            key1,key2 = twokey.split("_")
            if not key1 in working_dna or not key2 in working_dna: 
    ...
    try:
      for number_of_cluster in clusters.keys():
         try:
            ...
            if number_of_cluster not in groups: continue
...
def protein_2_dna_update(json_dict, output_dir, remote=False, verbose=True):
    ...
    json_dict["motifs"] = [groups.get(number_of_cluster)[:] for number_of_cluster in clusters.keys()]
