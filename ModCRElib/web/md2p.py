...
from ModCRElib.web.lib.src.JSONfile import dumpJSON, readJSON
...
def get_userdb(folder,fasta_file,mdl2fasta,uid_set,uid_db,label=None):
    ...
    # Built datasets
    # PWMs
    #print(homologs_dir,pwm_dir,database_pwm,uid_db)
    association={}
    pdb_db=set()
    ...
    for pwm_file in os.listdir(folder):
        #print("Check ",pwm_file, " in ", folder)
        ...
        try:
          pwm_file_wo_meme = ".".join( pwm_file.split(".")[:-1])
          pdb_chain        = pwm_file_wo_meme.split("_")[-3]+"_"+pwm_file_wo_meme.split("_")[-2]
        except:
          try:
            pdb_chain =".".join( pwm_file.split(".")[:-1])
          except:
            raise("Wrong format of PWM %s"%pwm_file)

        #print("pdb_chain",pdb_chain)
        if done:
           #print("Done",selected_uid,pdb_chain)
           association.setdefault(selected_uid,pdb_chain)
           pdb_db.add(pdb_chain)
        else:
           #print("NOT Done",pdb_chain,os.path.join(folder,pwm_file))
           msa_obj = PWM.nMSA(os.path.join(folder,pwm_file),pwm_file.rstrip(".meme"),"meme")
           if msa_obj.get_binding_site_length() >0 :
              #print("Add ",pwm_file, " to ", database_pwm)
              os.system("cat %s >> %s"%(os.path.join(folder,pwm_file),database_pwm))
              association.setdefault(".".join( pwm_file.split(".")[:-1]),pdb_chain)
              pdb_db.add(pdb_chain)
           else:
              print(("-- Skip PWM %s"%pwm_file))
    association_file = open(os.path.join(folder,"results","userdb2pdb.txt"),"w")
    #print("Association ",association)
    for pwm in list(association.keys()):
        association_file.write("%s\t%s\n"%(pwm,association[pwm]))
    association_file.close()
    # Sequences
    sequence_db = os.path.join(homologs_dir,"sequences_db.fa")
    if label is None or label == "model":
    ...
    autoreference={}
    for name,seq in functions.parse_fasta_file(sequence_db):
        autoreference.setdefault(name.upper(),seq)
    #print(autoreference)
    for pdb_chain in pdb_db:
        #print(("\t-- SEARCH HOMOLOGS OF: %s"%pdb_chain))
        homolog_file = os.path.join(homologs_dir,pdb_chain+".txt")
        if os.path.exists(os.path.join(pdb_dir,"split",pdb_chain+".fasta")):
           blast_obj = blast.get_blast_obj( sequence_db ,os.path.join(pdb_dir,"split",pdb_chain+".fasta"))
           #print(homolog_file,blast_obj.str_compacted_blast(tz_parameter=tz_parameter, tz_type=tz_type))
           functions.write(homolog_file, blast_obj.str_compacted_blast(tz_parameter=tz_parameter, tz_type=tz_type))
        else:
           if not os.path.exists(os.path.join(homologs_dir,pdb_chain+".fasta")):
              #print("Save",os.path.join(homologs_dir,pdb_chain+".fasta"),autoreference[pdb_chain.upper()])
              functions.write(os.path.join(homologs_dir,pdb_chain+".fasta"),">%s\n%s\n"%(pdb_chain,autoreference[pdb_chain.upper()]))
           blast_obj = blast.get_blast_obj( sequence_db ,os.path.join(homologs_dir,pdb_chain+".fasta"))
           #print(homolog_file,blast_obj.str_compacted_blast(tz_parameter=tz_parameter, tz_type=tz_type))
           functions.write(homolog_file, blast_obj.str_compacted_blast(tz_parameter=tz_parameter, tz_type=tz_type))
...
def get_pwm_database(json_dict, output_dir, dummy_dir, remote=True):
    ...
    if "custom_pwms" in json_dict and "custom_proteins" in json_dict:
    ...
    if "custom_proteins" not in json_dict: json_dict["custom_proteins"] = "null"


    selected_proteins = False
    if "prot" in json_dict:
...
def run_scan(json_dict, output_dir, database_external, database_dir, database_file, association_file, homologs_dir , scan_dir,  dummy_dir, remote=True):
    ...
    if "fimo_threshold" in json_dict:
    ...
    if os.path.isfile(os.path.join(scan_dir, "orthologs.json")):
        ...
        orthologs_list = json.loads(''.join([line for line in functions.parse_file(os.path.join(scan_dir, "orthologs.json"))]))
        if len(orthologs_list)<=0:
            json_dict["error_msg"] = "Scan without orthologs"
    else:
...
def get_protein_name(uniprot_id):
    ...
    prot_name = "Not found"
    try:
        html = urllib.request.urlopen("http://www.uniprot.org/uniprot/" + uniprot_id + ".txt")
...
def process_scan_outputs(json_dict,  output_dir, dummy_dir):
    ...
    if new_list == []:
       pickle_files=[]
       for  scsb in scan_subdir:
          for a in os.listdir(scsb):
              if a.endswith("pickle") and a.startswith("ortho") and os.path.getsize(os.path.join(scsb,a))!=0 :
                 pickle_files.append(os.path.join(scsb,a))
       if len(pickle_files)>0:
           json_dict["error_msg"] = "Too many binding sites to score in limitted time"
       else:
...
def handle_networks(full_json_dict, output_dir, dummy_dir,remote=False):
    ...
    for bs in list(input_prot_dict.keys()):
