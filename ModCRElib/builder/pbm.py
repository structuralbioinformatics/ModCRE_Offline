...
from ModCRElib.sequence import blast, nr, hmmer, homologs
    ...
    if options.start_step <= 1:
       if options.parallel:
        ...
        for linee in escores:
            ...
            break
        escores.close()
        for motif_name,columns in motif_names.items():
            escores=open(os.path.abspath(options.escores_file),"r")
            ...
            for line in escores:
                ...
                if len(values) > 0:
                   functions.write(motif_file, "%s;%s;%s" % (kmer, triads.get_complementary_dna_sequence(kmer), '{0:.3g}'.format(numpy.mean(list(map(float, values))))))
                else:
                ...
                if not os.path.exists(kmers_file):
                    ...
                    if len(positive_kmers) > 0:
                        ...
                        for line in functions.parse_file(pwm_file):
                            ...
                            position = line.pop(0)
                            if position != "Pos":
                                pwm.append(list(map(str, [float(j) * 100000 for j in line])))
                        # For each k-mer... #
                        for kmer in sorted(positive_kmers, key=lambda x: float(x[2]), reverse=True):
                            ...
                            for j in range(len(pwm) - 8 + 1):
                              try:
                                ...
                                sys.stdout.write("\t\t-- Skip motif %s test for %s: Error %s\n"%(tf_obj._motifs[i],tf_obj.get_id(),e))
                                continue
                            # For each hit... #
                    ...
                    if not os.path.exists(threading_file):
                        ...
                        try:
                          for position, score in motif.pssm.search(sequence, threshold=motif_score_threshold):
                            ...
                            sys.stdout.write("Error:  sequence hit %s cannot be analyzed %s\n"%(hit[0] + ".txt",e))
                            continue
                        ...
                        identities = sum([1 for i in range(len(list(hit[1]))) if hit[1][i]==hit[2][i]])
                        threading_protein = {}
                        # Get PDB file #
                        ...
                        positions = list(range(first, len(hit[2].replace("-", "")) + first))
        ...
        for nr_file in sorted(os.listdir(os.path.join(options.pdb_dir, "nr"))):
            ...
            if not os.path.exists(msa_file):
                ...
                if len(sequences) > 0:
                    ...
                    clustalo_path = os.path.join(src_path, config.get("Paths", "clustalo_path"))
                    try:
                      ...
                      os.system("%s -i %s -o %s --outfmt=stockholm "%(os.path.join(clustalo_path, "clustalo"),dummy_file,msa_file))
                    os.remove(dummy_file)
    # Exit if stops here #
    ...
    if options.start_step <= 5:
        ...
        if not os.path.exists(nr_file):
            ...
            if options.parallel:
                ...
                functions.submit_command_to_queue("%s %s %s --pdb=%s -r %s -o %s --pbm=%s  -t %s" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "nr.py"), label, options.pdb_dir, os.path.abspath(os.getcwd()), nr_file, options.output_dir,float(config.get("Parameters", "max_redundancy_general"))), cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
            # Else... #
            else:
                # Get non-redundant triads #
                nr_triads = nr.get_nr_triads(options.pdb_dir, threshold=float(config.get("Parameters", "max_redundancy_general")), pbm_dir=options.output_dir,non_redundant=non_redundant)
        ...
        for triads_file in os.listdir(os.path.join(options.pdb_dir, "triads")):
            ...
            if not os.path.exists(nr_file):
                ...
                if options.parallel:
                    ...
                    if non_redundant: label=" --nr "
                    functions.submit_command_to_queue("%s %s %s --pdb=%s -r %s -i %s -o %s --pbm=%s -t %s" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "nr.py"), label, options.pdb_dir, os.path.abspath(os.getcwd()), m.group(1), nr_file, options.output_dir, config.get("Parameters", "max_redundancy_family")), cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
                # Else... #
                else:
                    # Get non-redundant triads #
                    nr_triads = nr.get_nr_triads(options.pdb_dir, pdb_chain=m.group(1), threshold=float(config.get("Parameters", "max_redundancy_family")), pbm_dir=options.output_dir,non_redundant=non_redundant)
                    # For each nr triads object... #
                    for nr_triads_obj in nr_triads:
                        functions.write(nr_file, nr_triads_obj._file)
        ##############################
        # 5.3 Family group sets      #
        ##############################
        tfs_in_families = {}
        ##############################
        # 5.3.1 Parse motifs info      #
        ##############################
        if options.verbose: sys.stdout.write("\t\t-- parsing motif information...\n")
        # Initialize #
        motifs = {}
        ...
        for tf_family in tfs_in_families:
            ...
            list_file      = os.path.join(options.output_dir, "TFs_family_"+tf_family+".txt")
            ...
            nr_triads_file = os.path.join(options.output_dir, "nr", "family_"+tf_family+".txt")
            ...
            if not os.path.exists(folds_file):
               ...
               for threading_file in sorted(os.listdir(os.path.join(options.output_dir, "threading"))):
            ...
            if os.path.exists(nr_triads_file):
               if options.verbose: sys.stdout.write("\t\t\t-- use non-redundant triads of %s\n"%(nr_triads_file))
            else:
               if options.parallel:
                   # Submit to queue #
                   if  config.get("Cluster", "cluster_queue") == "None": cluster_queue=None
                   ...
                   if options.verbose: sys.stdout.write("Submit %s %s %s --pdb=%s -r %s -i %s -o %s --pbm=%s -t %s -l %s --dummy %s -v\n" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "nr.py"), label,options.pdb_dir, os.path.abspath(os.getcwd()), folds_name ,nr_triads_file , options.output_dir, config.get("Parameters", "max_redundancy_family"),list_file,dummy_folder))
                   functions.submit_command_to_queue("%s %s %s --pdb=%s -r %s -i %s -o %s --pbm=%s -t %s -l %s --dummy %s -v" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "nr.py"), label,options.pdb_dir, os.path.abspath(os.getcwd()), folds_name ,nr_triads_file , options.output_dir, config.get("Parameters", "max_redundancy_family"),list_file,dummy_folder), cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
               else:
                   nr_triads_family = nr.get_nr_triads(options.pdb_dir, threshold=float(config.get("Parameters", "max_redundancy_family")), pbm_dir=options.output_dir, list_file=list_file, non_redundant=non_redundant)
    ...
    if options.start_step <= 7:
        ...
        if not os.path.exists(families_file):
           if options.parallel:
            ...
            functions.submit_command_to_queue("%s %s --pdb=%s -o %s --dummy=%s -t %s -e %s -f %s -m %s --pwm %s -s %s -u %s --start=7 --stop=7 -v" % ( os.path.join(config.get("Paths", "python_path"), "python"),  os.path.join(scripts_path,os.path.basename(__file__)) , os.path.abspath(options.pdb_dir), os.path.abspath(options.output_dir) , os.path.abspath(options.dummy_dir), os.path.abspath(options.tfs_file), os.path.abspath(options.escores_file), os.path.abspath(options.families_file), os.path.abspath(options.motifs_file), os.path.abspath(options.pwm_dir), os.path.abspath(options.sources_file), os.path.abspath(options.uniprot_file)),cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
           else:
            # For each line... #
            ...
            if not os.path.exists(uniprot_file_hmm) or not  os.path.exists(uniprot_file_blast):
                ...
                if options.parallel:
                    ...
                    if os.path.exists(hmm_file) and not os.path.exists(uniprot_file_hmm) and homologs_hmm:
                        ...
                        functions.submit_command_to_queue("%s %s -d %s -i %s -f -o %s --dummy=%s" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "hmmer.py"), os.path.abspath(options.uniprot_file), os.path.abspath(hmm_file), os.path.abspath(homolog_file),options.dummy_dir), cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
                    # Get BLAST object #
                    if not os.path.exists(uniprot_file_blast):
                        ...
                        functions.submit_command_to_queue("%s %s -d %s -i %s -f -o %s --dummy=%s" % (os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "blast.py"), os.path.abspath(options.uniprot_file), os.path.abspath(fasta_file), os.path.abspath(homolog_file),options.dummy_dir), cluster_queue, int(config.get("Cluster", "max_jobs_in_queue")),os.path.join(scripts_path,config.get("Cluster","command_queue")),options.dummy_dir,config.get("Cluster","cluster_submit"),config.get("Cluster","cluster_qstat"))
                # Else... #
                else:
                    # If Hmmer file exists... #
                    hmm_file = os.path.join(options.output_dir, "hmm", pdb_chain + ".hmm")
        ...
        for homologs_file in sorted(os.listdir(os.path.join(options.output_dir, "uniprot"))):
          if (homologs_file.endswith(".blast") and not homologs_hmm) or (homologs_file.endswith(".hmm") and homologs_hmm):
            ...
            if not os.path.exists(dummy_file):
              print("%s %s -i %s -o %s --pdb=%s --pbm=%s --dummy=%s --filter" % ( os.path.join(config.get("Paths", "python_path"), "python"), os.path.join(scripts_path, "homologs.py"), os.path.join(os.path.abspath(options.output_dir), "uniprot",homologs_file), os.path.join(os.path.abspath(options.output_dir), "homologs", pdb_chain + ".txt"), os.path.abspath(options.pdb_dir), os.path.abspath(options.output_dir) , os.path.abspath(options.dummy_dir)))
