...
if not fileExist(members_storage) or not fileExist(cluster_storage):
 ...
 for hit in fimo_obj.get_hits(sort=True):
    ...
    if external is not None:
      if hit.get_hit() not in associated_pdb: continue
    ...
    for next_hit in fimo_obj.get_hits(sort=True):
        ...
        # Initialize #
        if external is not None:
           if next_hit.get_hit() not in associated_pdb: continue
        ...
        if next_fold_hit in dimers:
            ...
            for next_next_hit in fimo_obj.get_hits(sort=True):
                ...
                if external is not None:
                  if next_next_hit.get_hit() not in associated_pdb: continue
            ...
            if external is not None: 
                ...
                if dimer_fold_hit in associated_pdb_inv:
 ...
 for cluster in clusters:
    ...
    for members in cluster['members']:
        ...
        for i in range(2):
            ...
            if external is not None:
               if members[i].get_hit() not in associated_pdb: continue
            ...
            dna_helix=helix_hit
            # Get binding site #
            binding_site = list(range(interface_obj.get_start(), interface_obj.get_end() + 1))
            # Initialize #
            ...
            for ortholog in ortholog_hits.get_hits((0,None)):
                ...
                if external is not None:
                    if members[i].get_hit() in associated_pdb: 
                        fold_hit = associated_pdb.get(members[i].get_hit())
    ...
    for i in range(len(clusters)):
        ...
        for members in cluster['members']:
            ...
            for j in range(2):
                ...
                if external is not None:
                   if members[j].get_hit() not in associated_pdb: continue
...
# Initialize #
max_ratio_loss_scan = float(config.get("Parameters", "max_ratio_loss_scan"))
threaded = set()
...
for i in range(len(clusters)):
    ...
    for j in range(2):
        ...
        for ortholog in orthologs[i]['orthologs'][j]:
            ...
            if m:
                ...
                if m.group(1) in unique_orthologs[j]:
                  # If better score #
    ...
    # Initialize #
    matrix = [[[None], [None]] for j in range(max([len([iter(unique_orthologs[k].keys())]) for k in range(2)]))]
    # For each monomer... #
    #for j in range(len(orthologs[i]['orthologs'])):
    for j in range(2):
        ...
        for score in sorted(scores):
            for database in ["sp", "tr","SP","TR"]:
                # For each ortholog... #
                for ortholog in unique_orthologs[j].keys():
                    ...
                    if m:
                       ...
                       ortholog_database ="sp"
                       ortholog_name     =header
                       ortholog_genename =header
                       ortholog_specie   ="HUMAN"
    ...
    for j in range(2):
        ...
        for ortholog in orthologs[i]['orthologs'][j]:
            ...
            if m:
                ...
                if verbose: sys.stdout.write("ortholog %s does not match regular expression!\n"%ortholog[0])
    # Initialize #
    matrix = [[[None], [None]] for j in range(max([len([iter(all_orthologs[k].keys())]) for k in range(2)]))]
    # For each monomer... #
    for j in range(2):
        ...
        for score in sorted(scores):
            for database in ["sp", "tr","SP","TR"]:
                # For each ortholog... #
                for ortholog in all_orthologs[j].keys():
                    # Skip if ortholog does not belong to database or has a different score #
                    header = homologs[ortholog].sequenceID
                    m = re.search("^(tr|sp|TR|SP)\|(\S+)\|(\S+)\_(\S+)", header)
                    if m:
...
if not os.path.exists(os.path.join(output_dir,  "orthologs_complexes.json")):
  ...
  for pdb_hit,complex_list in complex_pdb_hits.items():
    ...
    cluster_hits={}
    for key, complex_number in cluster_complex.items():
  ...
  orthologs_complexes=[]
  complex_threaded={}
  for pdb_hit,cluster_hits in complex_cluster_hits.items():
   for name_cluster, complex_list in cluster_hits.items():
     tf_chain = {}
  ...
  for pdb_name_cluster,tf_chain in complex_threaded.items():
      chain_thread_files={}
      for chain,thread_files in tf_chain.items():
...
if not os.path.exists(os.path.join(output_dir,  "TF_clusters.json")):
 ...
 for ortholog_complex in orthologs_complexes:
     ...
     complex_threads = set()
     complex_hits    = set()
     for chain, thread_complex_files in ortholog_complex['chains'].items():
 ...
 for fragment in sorted(list(group_cluster.keys()), key=lambda x: int(x.split("_")[0])):
    binding        = fragment.split("_")
    range_fragment = set(range( int(binding[0]), int(binding[1])+1))
    if fragment in group_threads:
    ...
    for i in range(number_of_fragments+1):
        if i in merge_fragments:
           ...
           for fragment_done in merge_fragments[i]:
               ...
               if fragment_done in group_threads:
 ...
 for fragment in sorted(iter(merged_group_cluster.keys()), key=lambda x: int(x.split("_")[0])):
     binding= fragment.split("_")
     ...
     if fragment in merged_group_threads:
...
for tf_cluster in tf_clusters:
    ...
    for pdb,chains in tf_pdb_chains.items():
        out.write("\tPDB %s:\n"%pdb)
        for chain in chains:
            out.write("\tChain  %s:\n"%chain)
            pdb_chain=pdb+"_"+chain
            if pdb_chain in tf_group_threads_by_pdb:
