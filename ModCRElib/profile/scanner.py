...
if __name__ == "__main__":
    ...
    number_of_fragments = 1+int(len(dna_sequence)/step)
    dna_files = []
    dna_fragments = {}
    end   = 0
    extension=".txt"
    ...
    while(iterate):
        for start_index in list(dna_fragments.keys()):
        ...
        iterate = functions.check_done(done,dna_set) 
        #limit the maximum time to 3 days
        if (time() - start_time) > 175000: 
            sys.stdout.write("\t-- Exceeding time (limited to 2 days)\n")
            iterate=False
        if len(done) > n_done:
           n_done=len(done)
           if options.verbose: 
    ...
    for start_index in list(dna_fragments.keys()):
           ...
           try:
             ...
             sys.stdout.write("\t\t Warning: causing error %s\n"%(e))
    # 2) orthologs
    ...
    if not os.path.exists(os.path.join(output_dir,  "orthologs.json")):
       ...
       for start_index in list(dna_fragments.keys()):
           ...
           if not os.path.exists(os.path.join(folder,"orthologs.json")): continue
    ...
    if not os.path.exists(os.path.join(output_dir,  "orthologs.txt")):
       ...
       for start_index in list(dna_fragments.keys()):
           folder      = output_dir+"_"+str(start_index)
           if not os.path.exists(os.path.join(folder,"orthologs.txt")): continue
    ...
    if not os.path.exists(os.path.join(output_dir,  "orthologs_with_best_templates.json")):
       ...
       for start_index in list(dna_fragments.keys()):
           folder      = output_dir+"_"+str(start_index)
           if not os.path.exists(os.path.join(folder,"orthologs_with_best_templates.json")): continue
    ...
    if not os.path.exists(os.path.join(output_dir,  "orthologs_with_best_templates.txt")):
       ...
       for start_index in list(dna_fragments.keys()):
           folder      = output_dir+"_"+str(start_index)
           if not os.path.exists(os.path.join(folder,"orthologs_with_best_templates.txt")): continue
