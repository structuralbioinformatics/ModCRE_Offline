...
from ModCRElib.structure.protein import dssp, tmalign, model_protein
...
#Get the PWM
    if not functions.fileExist(output_file+".meme"):
           ...
           PWM.get_single_pwm(None,thread_file,True,output_file,pbm_dir,pdb_dir,families,potential_file, radius,fragment_restrict, binding_restrict, split_potential,auto_mode,family_potentials,pbm_potentials,score_threshold,taylor_approach,pmf,bins,known,meme,reset, refine, dummy_dir,verbose,methylation_pwm)
...
#Get the PWM
    if not functions.fileExist(output_file+".meme"):
           if verbose:sys.stdout.write("Generate PWM %s ...\n"%(output_file))
           PWM.get_single_pwm(pdb_file,None,False,output_file,pbm_dir,pdb_dir,families,potential_file, radius,fragment_restrict, binding_restrict, split_potential,auto_mode,family_potentials,pbm_potentials,score_threshold,taylor_approach,pmf,bins,known,meme,reset,refine, dummy_dir,verbose,methylation_pwm)
...
class Profile(object):
    ...
    def to_mean_table(self):
        ...
        for thr in thresholds:
          ...
          for potential in split_potentials:
            ...
            if len(y[abs(y)>1.0e-10])<1: continue
            tbl_dict.setdefault(potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=False,potential=potential).mean(0))
            tbl_dict.setdefault("normal_"+potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=True,potential=potential).mean(0))
            tbl_dict.setdefault(potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=False,potential=potential).mean(0))
            tbl_dict.setdefault("normal_"+potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=True,potential=potential).mean(0))
            tbl_dict.setdefault("best_"+potential+"_weighted_"+str(thr),self.get_energy_best_weighted_profile(thr,potential=potential).mean(0))
    ...
    def to_rmsd_table(self):
        ...
        for thr in thresholds:
         if thr is not None:
            tbl_dict.setdefault("binding_"+str(thr),self.get_fimo_binding_by_threshold(thr).std(0))
            tbl_dict.setdefault("fimo_score_"+str(thr),self.get_fimo_score_by_threshold(thr).std(0))
            tbl_dict.setdefault("fimo_log_score_"+str(thr),self.get_fimo_log_score_by_threshold(thr).std(0))
        for potential in split_potentials:
            y=self.get_energy_profile(normal=False,potential=potential).mean(0)
            if len(y[abs(y)>1.0e-10])<1: continue
        ...
        for thr in thresholds:
          ...
          for potential in split_potentials:
            y=self.get_energy_weighted_profile(thr,normal=False,potential=potential).mean(0)
            if len(y[abs(y)>1.0e-10])<1: continue
            tbl_dict.setdefault(potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=False,potential=potential).std(0))
            tbl_dict.setdefault("normal_"+potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=True,potential=potential).std(0))
            tbl_dict.setdefault(potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=False,potential=potential).std(0))
            tbl_dict.setdefault("normal_"+potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=True,potential=potential).std(0))
            tbl_dict.setdefault("best_"+potential+"_weighted_"+str(thr),self.get_energy_best_weighted_profile(thr,potential=potential).std(0))
        table=pd.DataFrame(tbl_dict)
        return table
    ...
    def plot_html(self,score_types,normal,energies,output,header_title=False):
        ...
        for plt_name in order_selection:
            ...
            arg_yaxis.setdefault('tickfont',dict(size=12))
            if "fimo" in plt_name:
        ...
        if header_title:
            ...
            layout=dict(title="PROFILE: "+name_profile,updatemenus=updatemenus,annotations=annotations,plot_bgcolor='rgba(0,0,0,0)',paper_bgcolor='rgba(0,0,0,0)',yaxis= dict(title = "Ratio", tickfont=dict(size=16)),xaxis=xaxis_dict)
        else:
            ...
            layout=dict(title="",updatemenus=updatemenus,annotations=annotations,plot_bgcolor='rgba(0,0,0,0)',paper_bgcolor='rgba(0,0,0,0)',yaxis= dict(title = "Ratio", tickfont=dict(size=16)),xaxis=xaxis_dict)
        # Create the plot #
...
class ProfileProtein(object):
    ...
    def set_fimo_scores_by_threshold(self, fimo_obj, threshold):
        ...
        if threshold in self._fimo_score:     self._fimo_score[threshold]=fimo_score
    ...
    def get_energy_profile(self,normal=False,potential="s3dc_dd"):
        ...
        for i in range(len(self._dna)-binding_length+1):
            ...
            if len(list(score_per_nucleotide.keys()))<=0:continue
    ...
    def to_table(self):
        ...
        tbl_dict.setdefault("Nucleotide",list(self._dna))
        for thr in thresholds:
            tbl_dict.setdefault("binding_"+str(thr),self.get_fimo_binding_by_threshold(thr))
            tbl_dict.setdefault("fimo_score_"+str(thr),self.get_fimo_score_by_threshold(thr))
            tbl_dict.setdefault("fimo_log_score_"+str(thr),self.get_fimo_log_score_by_threshold(thr))
        for potential in split_potentials:
        ...
        for thr in thresholds:
          for potential in split_potentials:
            ...
            if len(y[abs(y)>1.0e-10])<1: continue
            tbl_dict.setdefault(potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=False,potential=potential))
            tbl_dict.setdefault("normal_"+potential+"_weighted_"+str(thr),self.get_energy_weighted_profile(thr,normal=True,potential=potential))
            tbl_dict.setdefault(potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=False,potential=potential))
            tbl_dict.setdefault("normal_"+potential+"_x_nucleotide"+"_weighted_"+str(thr),self.get_energy_per_nucleotide_weighted_profile(thr,normal=True,potential=potential))
            tbl_dict.setdefault("best_"+potential+"_weighted_"+str(thr),self.get_energy_best_weighted_profile(thr,potential=potential))
        table=pd.DataFrame(tbl_dict)
        return table
